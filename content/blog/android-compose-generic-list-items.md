---
title: "Generic List Items in Compose"
slug: "android-compose-generic-list-items"
description: "Reusable LazyColumn item composables: keyed items, content types, and item-specific padding without recomposition storms."
datePublished: "2026-08-10"
dateModified: "2026-08-10"
tags: ["Android", "Jetpack Compose", "Lists"]
keywords: "generic list Compose, LazyColumn reusable item, contentType LazyColumn"
faq:
  - q: "When should Android teams adopt generic list items in compose?"
    a: "Adopt generic list items in compose when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with generic list items in compose?"
  ...
  - q: "How do I debug generic list items in compose issues in production?"
...
object AndroidComposeGenericListItemsFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-compose-generic-list-items_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for generic list items in compose
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-compose-generic-list-items` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get generic list items in compose wrong in predictable ways:
...
When generic list items in compose misbehaves in production:
...
Before enabling `android-compose-generic-list-items` for all users:
