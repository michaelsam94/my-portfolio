---
title: "ProGuard Rules for Jetpack Compose"
slug: "android-proguard-rules-compose"
description: "R8 keep rules for Compose, Kotlin serialization, and reflection — shrink release builds without runtime ClassNotFoundException."
datePublished: "2026-08-01"
dateModified: "2026-08-01"
tags: ["Android", "R8", "Jetpack Compose"]
keywords: "ProGuard Compose, R8 keep rules Kotlin, Compose obfuscation, shrink release Android"
faq:
  - q: "When should Android teams adopt proguard rules for jetpack compose?"
    a: "Adopt proguard rules for jetpack compose when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with proguard rules for jetpack compose?"
  ...
  - q: "How do I debug proguard rules for jetpack compose issues in production?"
...
object AndroidProguardRulesComposeFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-proguard-rules-compose_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for proguard rules for jetpack compose
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-proguard-rules-compose` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get proguard rules for jetpack compose wrong in predictable ways:
...
When proguard rules for jetpack compose misbehaves in production:
...
Before enabling `android-proguard-rules-compose` for all users:
