---
title: "API CORS Preflight in Production"
slug: "api-cors-preflight-production"
description: "Configure Access-Control-Allow-* correctly — credentials mode, wildcard pitfalls, and preflight cache."
...
tags:
  ...
  - "API"
  - "HTTP"
keywords: "api cors preflight production, production, backend"
faq:
  - q: "What problem does API CORS Preflight in Production solve?"
    a: "It addresses production gaps teams hit when scaling api cors preflight production: correctness under concurrency, operability, and measurable SLOs instead of ad-hoc scripts."
  - q: "When should I adopt this pattern?"
    a: "Adopt when api cors preflight production appears on incident timelines, p95 latency regresses, or the next traffic doubling will break the current shortcut."
...
A billing service lost duplicate events because api cors preflight production was handled only in application code without database-enforced invariants. The fix was not more logging — it was moving the guarantee to the layer that survives process crashes and duplicate deliveries.

Senior backend work on api cors preflight in production is less about syntax and more about failure modes: what happens on retry, on partial outage, and when two deploy versions run simultaneously during a rolling update.
...
Dashboards for api cors preflight production should answer: 'Is the system slow, broken, or overloaded?' without SSH. Exemplars link spikes to trace IDs.
...
Measure before optimizing api cors preflight production. Capture baseline p50/p95 latency, error rate, and resource utilization under representative load. Change one variable at a time — pool size, batch size, timeout, cache TTL — and re-measure.
...
Ship api cors preflight production changes behind feature flags when behavior crosses service boundaries. Use canary deploys with automatic rollback on error rate or latency regression.
