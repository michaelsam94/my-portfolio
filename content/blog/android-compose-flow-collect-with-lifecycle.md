---
title: "Flow Collection With Lifecycle in Compose"
slug: "android-compose-flow-collect-with-lifecycle"
description: "repeatOnLifecycle in Compose: LaunchedEffect + lifecycleOwner, and avoiding duplicate collectors."
datePublished: "2026-08-13"
dateModified: "2026-08-13"
tags: ["Android", "Jetpack Compose", "Coroutines"]
keywords: "repeatOnLifecycle Compose, Flow collect lifecycle, LaunchedEffect lifecycle Flow"
faq:
  - q: "When should Android teams adopt flow collection with lifecycle in compose?"
    a: "Adopt flow collection with lifecycle in compose when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with flow collection with lifecycle in compose?"
  ...
  - q: "How do I debug flow collection with lifecycle in compose issues in production?"
...
object AndroidComposeFlowCollectWithLifecycleFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-compose-flow-collect-with-lifecycle_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for flow collection with lifecycle in compose
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-compose-flow-collect-with-lifecycle` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get flow collection with lifecycle in compose wrong in predictable ways:
...
When flow collection with lifecycle in compose misbehaves in production:
...
Before enabling `android-compose-flow-collect-with-lifecycle` for all users:
