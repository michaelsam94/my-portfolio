---
title: "Target Size Minimums in Touch Interfaces"
slug: "accessibility-target-size-minimum"
description: "WCAG 2.2 target size AA requires 24px minimum — spacing patterns for dense dashboards without breaking layout."
datePublished: "2026-08-03"
dateModified: "2026-08-03"
tags: ["Accessibility", "WCAG 2.2", "Mobile"]
keywords: "target size WCAG 2.2, touch target minimum, mobile accessibility"
faq:
  - q: "What is Target Size Minimums in Touch Interfaces?"
    a: "Target Size Minimums in Touch Interfaces is a production pattern for frontend and product engineering teams building performant, accessible web applications. It addresses real constraints around user experience, security, and measurable outcomes — not theoretical best practices disconnected from shipping code."
  - q: "When should teams adopt Target Size Minimums in Touch Interfaces?"
    a: "Adopt Target Size Minimums in Touch Interfaces when you have field data or user research showing pain — slow interactions, accessibility gaps, conversion drop-offs, or security findings — and simpler fixes have been exhausted. Pilot on one route or feature before rolling out platform-wide."
  - q: "What are common mistakes with Target Size Minimums in Touch Interfaces?"
...
Document which metrics you expect to move. If target size minimums in touch interfaces is a performance change, baseline LCP, INP, and CLS in CrUX or your RUM tool for affected routes before merging. If it is an accessibility change, run axe and manual screen reader checks on the critical path — not just the component story.

## Implementation patterns

Start with the smallest change that proves the approach. For target size minimums in touch interfaces, that usually means one route, one component tree, or one middleware rule — not a platform-wide migration.

```tsx
// Example: progressive adoption pattern
// Step 1 — isolate behind a feature flag or route segment
export async function Page() {
  const enabled = await flags.isEnabled("accessibility_target_size_minimum");
...
Teams get target size minimums in touch interfaces wrong in predictable ways:
...
When target size minimums in touch interfaces misbehaves in production, work top-down:
