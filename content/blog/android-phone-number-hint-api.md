...
description: "Google Play services Phone Number Hint: reduce friction at signup without READ_PHONE_STATE or SMS permissions."
datePublished: "2026-07-17"
dateModified: "2026-07-17"
tags: ["Android", "Identity", "UX"]
keywords: "Phone Number Hint API, Identity API Android, signup friction reduction"
faq:
  - q: "When should Android teams adopt phone number hint api integration?"
    a: "Adopt phone number hint api integration when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with phone number hint api integration?"
  ...
  - q: "How do I debug phone number hint api integration issues in production?"
...
object AndroidPhoneNumberHintApiFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-phone-number-hint-api_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for phone number hint api integration
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-phone-number-hint-api` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get phone number hint api integration wrong in predictable ways:
...
When phone number hint api integration misbehaves in production:
...
Before enabling `android-phone-number-hint-api` for all users:
