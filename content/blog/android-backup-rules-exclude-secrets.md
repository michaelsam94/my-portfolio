---
title: "Backup Rules to Exclude Secrets"
slug: "android-backup-rules-exclude-secrets"
description: "Configure backup_rules.xml and data_extraction_rules: exclude SharedPreferences tokens, Keystore, and database keys."
datePublished: "2026-08-15"
dateModified: "2026-08-15"
tags: ["Android", "Security", "Backup"]
keywords: "backup rules Android, data extraction rules, exclude secrets backup Android"
faq:
  - q: "When should Android teams adopt backup rules to exclude secrets?"
    a: "Adopt backup rules to exclude secrets when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with backup rules to exclude secrets?"
  ...
  - q: "How do I debug backup rules to exclude secrets issues in production?"
...
object AndroidBackupRulesExcludeSecretsFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-backup-rules-exclude-secrets_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for backup rules to exclude secrets
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-backup-rules-exclude-secrets` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get backup rules to exclude secrets wrong in predictable ways:
...
When backup rules to exclude secrets misbehaves in production:
...
Before enabling `android-backup-rules-exclude-secrets` for all users:
