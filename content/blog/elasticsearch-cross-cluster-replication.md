---
title: "Elasticsearch Cross Cluster Replication"
slug: "elasticsearch-cross-cluster-replication"
description: "Follower indices for DR — bi-directional limitations and failover steps."
...
tags:
  ...
  - "Backend"
  - "Search"
keywords: "elasticsearch cross cluster replication, production, backend"
faq:
  - q: "What problem does Elasticsearch Cross Cluster Replication solve?"
    a: "It addresses production gaps teams hit when scaling elasticsearch cross cluster replication: correctness under concurrency, operability, and measurable SLOs instead of ad-hoc scripts."
  - q: "When should I adopt this pattern?"
    a: "Adopt when elasticsearch cross cluster replication appears on incident timelines, p95 latency regresses, or the next traffic doubling will break the current shortcut."
...
A billing service lost duplicate events because elasticsearch cross cluster replication was handled only in application code without database-enforced invariants. The fix was not more logging — it was moving the guarantee to the layer that survives process crashes and duplicate deliveries.

Senior backend work on elasticsearch cross cluster replication is less about syntax and more about failure modes: what happens on retry, on partial outage, and when two deploy versions run simultaneously during a rolling update.
...
Dashboards for elasticsearch cross cluster replication should answer: 'Is the system slow, broken, or overloaded?' without SSH. Exemplars link spikes to trace IDs.
...
Measure before optimizing elasticsearch cross cluster replication. Capture baseline p50/p95 latency, error rate, and resource utilization under representative load. Change one variable at a time — pool size, batch size, timeout, cache TTL — and re-measure.
...
Ship elasticsearch cross cluster replication changes behind feature flags when behavior crosses service boundaries. Use canary deploys with automatic rollback on error rate or latency regression.
