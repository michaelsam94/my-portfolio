---
title: "Popup and Dropdown Positioning"
slug: "android-compose-popup-dropdown-positioning"
description: "DropdownMenu popupPositionProvider: avoid clipping, RTL alignment, and keyboard overlap adjustment."
datePublished: "2026-07-27"
dateModified: "2026-07-27"
tags: ["Android", "Jetpack Compose", "UI"]
keywords: "DropdownMenu positioning Compose, popupPositionProvider, menu clipping Compose"
faq:
  - q: "When should Android teams adopt popup and dropdown positioning?"
    a: "Adopt popup and dropdown positioning when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with popup and dropdown positioning?"
  ...
  - q: "How do I debug popup and dropdown positioning issues in production?"
...
object AndroidComposePopupDropdownPositioningFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-compose-popup-dropdown-positioning_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for popup and dropdown positioning
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-compose-popup-dropdown-positioning` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get popup and dropdown positioning wrong in predictable ways:
...
When popup and dropdown positioning misbehaves in production:
...
Before enabling `android-compose-popup-dropdown-positioning` for all users:
