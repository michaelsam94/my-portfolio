---
title: "Hilt Test Rule for Compose Tests"
slug: "android-compose-test-rule-hilt"
description: "HiltAndroidTest with createAndroidComposeRule: replace bindings, test ViewModels, and fake repositories."
datePublished: "2026-08-09"
dateModified: "2026-08-09"
tags: ["Android", "Testing", "Hilt"]
keywords: "Hilt Compose test, createAndroidComposeRule Hilt, Android instrumented test Hilt"
faq:
  - q: "When should Android teams adopt hilt test rule for compose tests?"
    a: "Adopt hilt test rule for compose tests when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with hilt test rule for compose tests?"
  ...
  - q: "How do I debug hilt test rule for compose tests issues in production?"
...
object AndroidComposeTestRuleHiltFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-compose-test-rule-hilt_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for hilt test rule for compose tests
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-compose-test-rule-hilt` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get hilt test rule for compose tests wrong in predictable ways:
...
When hilt test rule for compose tests misbehaves in production:
...
Before enabling `android-compose-test-rule-hilt` for all users:
