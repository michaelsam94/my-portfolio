---
title: "Stale-While-Revalidate in Service Workers"
slug: "web-performance-service-worker-stale-while-revalidate"
description: "SWR serves cached instantly then updates — stale-while-revalidate header and SW strategy alignment."
datePublished: "2027-02-05"
dateModified: "2027-02-05"
tags: ["Performance", "PWA", "Caching"]
keywords: "stale while revalidate service worker, SWR caching, Cache-Control stale-while-revalidate"
faq:
  - q: "What is Stale-While-Revalidate in Service Workers?"
    a: "Stale-While-Revalidate in Service Workers is a production pattern for frontend and product engineering teams building performant, accessible web applications. It addresses real constraints around user experience, security, and measurable outcomes — not theoretical best practices disconnected from shipping code."
  - q: "When should teams adopt Stale-While-Revalidate in Service Workers?"
    a: "Adopt Stale-While-Revalidate in Service Workers when you have field data or user research showing pain — slow interactions, accessibility gaps, conversion drop-offs, or security findings — and simpler fixes have been exhausted. Pilot on one route or feature before rolling out platform-wide."
  - q: "What are common mistakes with Stale-While-Revalidate in Service Workers?"
...
Document which metrics you expect to move. If stale-while-revalidate in service workers is a performance change, baseline LCP, INP, and CLS in CrUX or your RUM tool for affected routes before merging. If it is an accessibility change, run axe and manual screen reader checks on the critical path — not just the component story.

## Implementation patterns

Start with the smallest change that proves the approach. For stale-while-revalidate in service workers, that usually means one route, one component tree, or one middleware rule — not a platform-wide migration.

```tsx
// Example: progressive adoption pattern
// Step 1 — isolate behind a feature flag or route segment
export async function Page() {
  const enabled = await flags.isEnabled("web_performance_service_worker_stale_while_revalidate");
...
Teams get stale-while-revalidate in service workers wrong in predictable ways:
...
When stale-while-revalidate in service workers misbehaves in production, work top-down:
