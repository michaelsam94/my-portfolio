...
description: "Flow control on bidi streams — application-level pause/resume vs channel buffer exhaustion."
...
tags:
  ...
  - "Backend"
  - "API"
keywords: "grpc bidirectional stream backpressure, production, backend"
faq:
  - q: "What problem does gRPC Bidirectional Stream Backpressure solve?"
    a: "It addresses production gaps teams hit when scaling grpc bidirectional stream backpressure: correctness under concurrency, operability, and measurable SLOs instead of ad-hoc scripts."
  - q: "When should I adopt this pattern?"
    a: "Adopt when grpc bidirectional stream backpressure appears on incident timelines, p95 latency regresses, or the next traffic doubling will break the current shortcut."
...
A billing service lost duplicate events because grpc bidirectional stream backpressure was handled only in application code without database-enforced invariants. The fix was not more logging — it was moving the guarantee to the layer that survives process crashes and duplicate deliveries.

Senior backend work on grpc bidirectional stream backpressure is less about syntax and more about failure modes: what happens on retry, on partial outage, and when two deploy versions run simultaneously during a rolling update.
...
Dashboards for grpc bidirectional stream backpressure should answer: 'Is the system slow, broken, or overloaded?' without SSH. Exemplars link spikes to trace IDs.
...
Measure before optimizing grpc bidirectional stream backpressure. Capture baseline p50/p95 latency, error rate, and resource utilization under representative load. Change one variable at a time — pool size, batch size, timeout, cache TTL — and re-measure.
...
Ship grpc bidirectional stream backpressure changes behind feature flags when behavior crosses service boundaries. Use canary deploys with automatic rollback on error rate or latency regression.
