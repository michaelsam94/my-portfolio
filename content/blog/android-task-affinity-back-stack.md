---
title: "Task Affinity and Back Stack Control"
slug: "android-task-affinity-back-stack"
description: "taskAffinity, allowTaskReparenting, and clearing tasks: multi-task apps, document launch mode, and recents UX."
datePublished: "2026-07-24"
dateModified: "2026-07-24"
tags: ["Android", "Activities", "Tasks"]
keywords: "task affinity Android, back stack control, allowTaskReparenting Android"
faq:
  - q: "When should Android teams adopt task affinity and back stack control?"
    a: "Adopt task affinity and back stack control when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with task affinity and back stack control?"
  ...
  - q: "How do I debug task affinity and back stack control issues in production?"
...
object AndroidTaskAffinityBackStackFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-task-affinity-back-stack_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for task affinity and back stack control
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-task-affinity-back-stack` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get task affinity and back stack control wrong in predictable ways:
...
When task affinity and back stack control misbehaves in production:
...
Before enabling `android-task-affinity-back-stack` for all users:
