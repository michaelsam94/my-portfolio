---
title: "Full-Screen Dialog in Compose"
slug: "android-compose-fullscreen-dialog"
description: "Dialog with fillMaxSize: immersive flows, predictive back, and replacing DialogFragment migration path."
datePublished: "2026-07-26"
dateModified: "2026-07-26"
tags: ["Android", "Jetpack Compose", "Navigation"]
keywords: "fullscreen dialog Compose, Dialog fillMaxSize, DialogFragment migration Compose"
faq:
  - q: "When should Android teams adopt full-screen dialog in compose?"
    a: "Adopt full-screen dialog in compose when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with full-screen dialog in compose?"
  ...
  - q: "How do I debug full-screen dialog in compose issues in production?"
...
object AndroidComposeFullscreenDialogFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-compose-fullscreen-dialog_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for full-screen dialog in compose
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-compose-fullscreen-dialog` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get full-screen dialog in compose wrong in predictable ways:
...
When full-screen dialog in compose misbehaves in production:
...
Before enabling `android-compose-fullscreen-dialog` for all users:
