---
title: "JankStats Integration with Compose"
slug: "android-compose-jankstats-integration"
description: "FrameMetricsReporter with Compose: detect janky frames, tag UI surfaces, and correlate with recomposition counts."
datePublished: "2026-07-18"
dateModified: "2026-07-18"
tags: ["Android", "Jetpack Compose", "Performance"]
keywords: "JankStats Compose, frame jank Compose, Compose performance monitoring"
faq:
  - q: "When should Android teams adopt jankstats integration with compose?"
    a: "Adopt jankstats integration with compose when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with jankstats integration with compose?"
  ...
  - q: "How do I debug jankstats integration with compose issues in production?"
...
object AndroidComposeJankstatsIntegrationFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-compose-jankstats-integration_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for jankstats integration with compose
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-compose-jankstats-integration` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get jankstats integration with compose wrong in predictable ways:
...
When jankstats integration with compose misbehaves in production:
...
Before enabling `android-compose-jankstats-integration` for all users:
