---
title: "Play Integrity Standard Request Flow"
slug: "android-play-integrity-standard-request"
description: "Standard integrity token requests: cloud project number, retry on transient errors, and server decoding."
datePublished: "2026-08-13"
dateModified: "2026-08-13"
tags: ["Android", "Play Integrity", "Security"]
keywords: "Play Integrity standard request, integrity token Android, Play Integrity API flow"
faq:
  - q: "When should Android teams adopt play integrity standard request flow?"
    a: "Adopt play integrity standard request flow when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with play integrity standard request flow?"
  ...
  - q: "How do I debug play integrity standard request flow issues in production?"
...
object AndroidPlayIntegrityStandardRequestFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-play-integrity-standard-request_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for play integrity standard request flow
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-play-integrity-standard-request` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get play integrity standard request flow wrong in predictable ways:
...
When play integrity standard request flow misbehaves in production:
...
Before enabling `android-play-integrity-standard-request` for all users:
