---
title: "WindowSizeClass in Compose"
slug: "android-compose-window-size-class"
description: "CalculateWindowSizeClass: Compact/Medium/Expanded width and height, orientation, and responsive typography."
datePublished: "2026-07-29"
dateModified: "2026-07-29"
tags: ["Android", "Jetpack Compose", "Adaptive"]
keywords: "WindowSizeClass Compose, responsive Compose layout, compact medium expanded"
faq:
  - q: "When should Android teams adopt windowsizeclass in compose?"
    a: "Adopt windowsizeclass in compose when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with windowsizeclass in compose?"
  ...
  - q: "How do I debug windowsizeclass in compose issues in production?"
...
object AndroidComposeWindowSizeClassFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-compose-window-size-class_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for windowsizeclass in compose
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-compose-window-size-class` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get windowsizeclass in compose wrong in predictable ways:
...
When windowsizeclass in compose misbehaves in production:
...
Before enabling `android-compose-window-size-class` for all users:
