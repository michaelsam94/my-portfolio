---
title: "BOOT_COMPLETED Receiver Limits"
slug: "android-boot-completed-receiver-limits"
description: "Restricted boot receivers on Android 15+: direct boot, lazy initialization, and WorkManager deferral after boot."
datePublished: "2026-07-19"
dateModified: "2026-07-19"
tags: ["Android", "BroadcastReceiver", "Background"]
keywords: "BOOT_COMPLETED Android, boot receiver restrictions, direct boot aware Android"
faq:
  - q: "When should Android teams adopt boot_completed receiver limits?"
    a: "Adopt boot_completed receiver limits when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with boot_completed receiver limits?"
  ...
  - q: "How do I debug boot_completed receiver limits issues in production?"
...
object AndroidBootCompletedReceiverLimitsFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-boot-completed-receiver-limits_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for boot_completed receiver limits
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-boot-completed-receiver-limits` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get boot_completed receiver limits wrong in predictable ways:
...
When boot_completed receiver limits misbehaves in production:
...
Before enabling `android-boot-completed-receiver-limits` for all users:
