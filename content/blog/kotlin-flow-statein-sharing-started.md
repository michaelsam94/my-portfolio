---
title: "StateIn SharingStarted Policies on Android"
slug: "kotlin-flow-statein-sharing-started"
description: "WhileSubscribed(5000) vs Eagerly: screen lifecycle, config change, and multi-collector ViewModels."
datePublished: "2026-07-22"
dateModified: "2026-07-22"
tags: ["Kotlin", "Flow", "Android"]
keywords: "SharingStarted WhileSubscribed, stateIn ViewModel Android, Flow stateIn policy"
faq:
  - q: "When should Android teams adopt statein sharingstarted policies on android?"
    a: "Adopt statein sharingstarted policies on android when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with statein sharingstarted policies on android?"
  ...
  - q: "How do I debug statein sharingstarted policies on android issues in production?"
...
object KotlinFlowStateinSharingStartedFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("kotlin-flow-statein-sharing-started_enabled", default = false)
}

class KotlinRepository @Inject constructor(
    private val dispatcher: CoroutineDispatcher = Dispatchers.IO,
) {
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for statein sharingstarted policies on android
        }
    }
}
```
...
class ExampleViewModel @Inject constructor(
    private val repo: KotlinRepository,
) : ViewModel() {
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `kotlin-flow-statein-sharing-started` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get statein sharingstarted policies on android wrong in predictable ways:
...
When statein sharingstarted policies on android misbehaves in production:
...
Before enabling `kotlin-flow-statein-sharing-started` for all users:
