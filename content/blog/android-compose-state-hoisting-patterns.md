---
title: "State Hoisting Patterns in Compose"
slug: "android-compose-state-hoisting-patterns"
description: "Stateful vs stateless composables: hoisting, unidirectional data flow, and event callbacks up the tree."
datePublished: "2026-08-07"
dateModified: "2026-08-07"
tags: ["Android", "Jetpack Compose", "State"]
keywords: "state hoisting Compose, stateless composable, Compose UDF patterns"
faq:
  - q: "When should Android teams adopt state hoisting patterns in compose?"
    a: "Adopt state hoisting patterns in compose when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with state hoisting patterns in compose?"
  ...
  - q: "How do I debug state hoisting patterns in compose issues in production?"
...
object AndroidComposeStateHoistingPatternsFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-compose-state-hoisting-patterns_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for state hoisting patterns in compose
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-compose-state-hoisting-patterns` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get state hoisting patterns in compose wrong in predictable ways:
...
When state hoisting patterns in compose misbehaves in production:
...
Before enabling `android-compose-state-hoisting-patterns` for all users:
