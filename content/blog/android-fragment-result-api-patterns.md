---
title: "Fragment Result API Patterns"
slug: "android-fragment-result-api-patterns"
description: "Pass results between fragments with Fragment Result API — replacing target fragments and avoiding stale listener leaks."
datePublished: "2026-07-28"
dateModified: "2026-07-28"
tags: ["Android", "Fragments", "Navigation"]
keywords: "Fragment Result API, setFragmentResult, fragment communication Android"
faq:
  - q: "When should Android teams adopt fragment result api patterns?"
    a: "Adopt fragment result api patterns when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with fragment result api patterns?"
  ...
  - q: "How do I debug fragment result api patterns issues in production?"
...
object AndroidFragmentResultApiPatternsFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-fragment-result-api-patterns_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for fragment result api patterns
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-fragment-result-api-patterns` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get fragment result api patterns wrong in predictable ways:
...
When fragment result api patterns misbehaves in production:
...
Before enabling `android-fragment-result-api-patterns` for all users:
