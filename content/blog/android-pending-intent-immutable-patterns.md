...
description: "Safe PendingIntent creation: request codes, explicit intents, and avoiding intent hijacking with immutable flags."
datePublished: "2026-07-22"
dateModified: "2026-07-22"
tags: ["Android", "Security", "PendingIntent"]
keywords: "immutable PendingIntent patterns, PendingIntent security Android, explicit intent PendingIntent"
faq:
  - q: "When should Android teams adopt pendingintent immutable patterns?"
    a: "Adopt pendingintent immutable patterns when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with pendingintent immutable patterns?"
  ...
  - q: "How do I debug pendingintent immutable patterns issues in production?"
...
object AndroidPendingIntentImmutablePatternsFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-pending-intent-immutable-patterns_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for pendingintent immutable patterns
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-pending-intent-immutable-patterns` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get pendingintent immutable patterns wrong in predictable ways:
...
When pendingintent immutable patterns misbehaves in production:
...
Before enabling `android-pending-intent-immutable-patterns` for all users:
