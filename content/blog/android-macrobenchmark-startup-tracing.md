...
description: "Measure cold/warm/hot startup: StartupTimingMetric, BaselineProfileRule, and comparing releases in CI."
datePublished: "2026-07-17"
dateModified: "2026-07-17"
tags: ["Android", "Performance", "Benchmark"]
keywords: "Macrobenchmark startup, StartupTimingMetric Android, cold start benchmark"
faq:
  - q: "When should Android teams adopt macrobenchmark startup tracing?"
    a: "Adopt macrobenchmark startup tracing when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with macrobenchmark startup tracing?"
  ...
  - q: "How do I debug macrobenchmark startup tracing issues in production?"
...
object AndroidMacrobenchmarkStartupTracingFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-macrobenchmark-startup-tracing_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for macrobenchmark startup tracing
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-macrobenchmark-startup-tracing` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get macrobenchmark startup tracing wrong in predictable ways:
...
When macrobenchmark startup tracing misbehaves in production:
...
Before enabling `android-macrobenchmark-startup-tracing` for all users:
