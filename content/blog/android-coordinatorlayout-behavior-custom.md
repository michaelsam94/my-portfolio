---
title: "Custom CoordinatorLayout Behaviors"
slug: "android-coordinatorlayout-behavior-custom"
description: "Write custom Behavior classes for CoordinatorLayout: scroll-linked toolbars, FAB hide-on-scroll, and nested scrolling fixes."
datePublished: "2026-07-30"
dateModified: "2026-07-30"
tags: ["Android", "UI", "Views"]
keywords: "CoordinatorLayout Behavior, custom Behavior Android, AppBarLayout scroll"
faq:
  - q: "When should Android teams adopt custom coordinatorlayout behaviors?"
    a: "Adopt custom coordinatorlayout behaviors when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with custom coordinatorlayout behaviors?"
  ...
  - q: "How do I debug custom coordinatorlayout behaviors issues in production?"
...
object AndroidCoordinatorlayoutBehaviorCustomFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-coordinatorlayout-behavior-custom_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for custom coordinatorlayout behaviors
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-coordinatorlayout-behavior-custom` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get custom coordinatorlayout behaviors wrong in predictable ways:
...
When custom coordinatorlayout behaviors misbehaves in production:
...
Before enabling `android-coordinatorlayout-behavior-custom` for all users:
...
## Coordinatorlayout Behavior Custom on Samsung and Pixel divergence

Exercise coordinatorlayout behavior custom on Galaxy A-series and Pixel a-series — emulators hide OEM battery and storage quirks. Capture Macrobenchmark or Firebase trace for the critical path touching coordinatorlayout; regressions above 8% block release for `android-coordinatorlayout-behavior-custom`.

Document permission and background behavior in internal runbook: what breaks under Doze, what requires foreground service, and what Play policy declarations apply. Support tickets referencing "Coordinatorlayout Behavior Custom" should map to a single runbook section with known workarounds.

## Behavior regression gates for Play Vitals

Before promoting `android-coordinatorlayout-behavior-custom` changes past 20% rollout, compare ANR rate, slow cold start, and excessive wakeups against seven-day baseline. Fail rollback review if custom path shows >5% increase in `slow frames` without documented trade-off approval.
