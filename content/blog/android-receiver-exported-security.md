---
title: "Broadcast Receiver Exported Security"
slug: "android-receiver-exported-security"
description: "android:exported audit: implicit intents, signature permissions, and LocalBroadcastManager replacements."
datePublished: "2026-07-20"
dateModified: "2026-07-20"
tags: ["Android", "Security", "BroadcastReceiver"]
keywords: "exported receiver security, BroadcastReceiver exported Android, implicit broadcast"
faq:
  - q: "When should Android teams adopt broadcast receiver exported security?"
    a: "Adopt broadcast receiver exported security when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with broadcast receiver exported security?"
  ...
  - q: "How do I debug broadcast receiver exported security issues in production?"
...
object AndroidReceiverExportedSecurityFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-receiver-exported-security_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for broadcast receiver exported security
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-receiver-exported-security` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get broadcast receiver exported security wrong in predictable ways:
...
When broadcast receiver exported security misbehaves in production:
...
Before enabling `android-receiver-exported-security` for all users:
