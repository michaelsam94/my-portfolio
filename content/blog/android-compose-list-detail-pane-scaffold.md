---
title: "List-Detail Pane Scaffold in Compose"
slug: "android-compose-list-detail-pane-scaffold"
description: "ListDetailPaneScaffold: two-pane on tablet, single pane on phone, and state restoration across form factors."
datePublished: "2026-07-27"
dateModified: "2026-07-27"
tags: ["Android", "Jetpack Compose", "Adaptive"]
keywords: "ListDetailPaneScaffold Compose, two pane layout Android, adaptive list detail"
faq:
  - q: "When should Android teams adopt list-detail pane scaffold in compose?"
    a: "Adopt list-detail pane scaffold in compose when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with list-detail pane scaffold in compose?"
  ...
  - q: "How do I debug list-detail pane scaffold in compose issues in production?"
...
object AndroidComposeListDetailPaneScaffoldFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-compose-list-detail-pane-scaffold_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for list-detail pane scaffold in compose
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-compose-list-detail-pane-scaffold` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get list-detail pane scaffold in compose wrong in predictable ways:
...
When list-detail pane scaffold in compose misbehaves in production:
...
Before enabling `android-compose-list-detail-pane-scaffold` for all users:
