---
title: "collectAsStateWithLifecycle in Compose"
slug: "android-compose-collect-as-state-lifecycle"
description: "Replace collectAsState with lifecycle-aware collection: STARTED vs RESUMED, and stopping collectors in background."
datePublished: "2026-08-12"
dateModified: "2026-08-12"
tags: ["Android", "Jetpack Compose", "Lifecycle"]
keywords: "collectAsStateWithLifecycle, lifecycle aware Flow Compose, Flow collection lifecycle"
faq:
  - q: "When should Android teams adopt collectasstatewithlifecycle in compose?"
    a: "Adopt collectasstatewithlifecycle in compose when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with collectasstatewithlifecycle in compose?"
  ...
  - q: "How do I debug collectasstatewithlifecycle in compose issues in production?"
...
object AndroidComposeCollectAsStateLifecycleFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-compose-collect-as-state-lifecycle_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for collectasstatewithlifecycle in compose
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-compose-collect-as-state-lifecycle` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get collectasstatewithlifecycle in compose wrong in predictable ways:
...
When collectasstatewithlifecycle in compose misbehaves in production:
...
Before enabling `android-compose-collect-as-state-lifecycle` for all users:
