---
title: "Baseline Profiles for Compose Apps"
slug: "android-compose-performance-baseline-profile"
description: "Generate baseline profiles for Compose startup: Macrobenchmark, profileinstaller, and dex layout optimization."
datePublished: "2026-07-17"
dateModified: "2026-07-17"
tags: ["Android", "Jetpack Compose", "Performance"]
keywords: "baseline profile Compose, Compose startup optimization, profileinstaller Compose"
faq:
  - q: "When should Android teams adopt baseline profiles for compose apps?"
    a: "Adopt baseline profiles for compose apps when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with baseline profiles for compose apps?"
  ...
  - q: "How do I debug baseline profiles for compose apps issues in production?"
...
object AndroidComposePerformanceBaselineProfileFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-compose-performance-baseline-profile_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for baseline profiles for compose apps
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-compose-performance-baseline-profile` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get baseline profiles for compose apps wrong in predictable ways:
...
When baseline profiles for compose apps misbehaves in production:
...
Before enabling `android-compose-performance-baseline-profile` for all users:
