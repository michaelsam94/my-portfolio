---
title: "Task Hijacking and StrandHogg Prevention"
slug: "android-task-hijacking-prevention"
description: "Prevent task affinity hijacking: singleTask launch modes, allowTaskReparenting=false, and intent filter audit."
datePublished: "2026-08-09"
dateModified: "2026-08-09"
tags: ["Android", "Security", "Activities"]
keywords: "task hijacking Android, StrandHogg prevention, launch mode security"
faq:
  - q: "When should Android teams adopt task hijacking and strandhogg prevention?"
    a: "Adopt task hijacking and strandhogg prevention when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with task hijacking and strandhogg prevention?"
  ...
  - q: "How do I debug task hijacking and strandhogg prevention issues in production?"
...
object AndroidTaskHijackingPreventionFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-task-hijacking-prevention_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for task hijacking and strandhogg prevention
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-task-hijacking-prevention` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get task hijacking and strandhogg prevention wrong in predictable ways:
...
When task hijacking and strandhogg prevention misbehaves in production:
...
Before enabling `android-task-hijacking-prevention` for all users:
