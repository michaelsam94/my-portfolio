---
title: "Notification Bubbles API"
slug: "android-notification-bubbles-api"
description: "Implement conversation bubbles: BubbleMetadata, suppressBubble, and chat UX on Android 11+."
datePublished: "2026-08-05"
dateModified: "2026-08-05"
tags: ["Android", "Notifications", "UX"]
keywords: "notification bubbles Android, BubbleMetadata, conversation bubbles API"
faq:
  - q: "When should Android teams adopt notification bubbles api?"
    a: "Adopt notification bubbles api when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with notification bubbles api?"
  ...
  - q: "How do I debug notification bubbles api issues in production?"
...
object AndroidNotificationBubblesApiFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-notification-bubbles-api_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for notification bubbles api
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-notification-bubbles-api` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get notification bubbles api wrong in predictable ways:
...
When notification bubbles api misbehaves in production:
...
Before enabling `android-notification-bubbles-api` for all users:
