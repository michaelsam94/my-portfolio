---
title: "PendingIntent Mutability Flags"
slug: "android-pending-intent-mutability-flags"
description: "FLAG_IMMUTABLE vs FLAG_MUTABLE: fill-in intents, inline replies, and Android 12+ enforcement crashes."
datePublished: "2026-07-21"
dateModified: "2026-07-21"
tags: ["Android", "Security", "PendingIntent"]
keywords: "PendingIntent mutability, FLAG_IMMUTABLE Android, PendingIntent flags Android 12"
faq:
  - q: "When should Android teams adopt pendingintent mutability flags?"
    a: "Adopt pendingintent mutability flags when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with pendingintent mutability flags?"
  ...
  - q: "How do I debug pendingintent mutability flags issues in production?"
...
object AndroidPendingIntentMutabilityFlagsFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-pending-intent-mutability-flags_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for pendingintent mutability flags
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-pending-intent-mutability-flags` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get pendingintent mutability flags wrong in predictable ways:
...
When pendingintent mutability flags misbehaves in production:
...
Before enabling `android-pending-intent-mutability-flags` for all users:
