---
title: "Play Feature Delivery On-Demand Modules"
slug: "android-play-feature-delivery-on-demand"
description: "Configure on-demand and conditional modules: user country, device RAM, and install-time vs on-demand trade-offs."
datePublished: "2026-07-25"
dateModified: "2026-07-25"
tags: ["Android", "Play Store", "Modularization"]
keywords: "Play Feature Delivery, on-demand modules, conditional modules Android"
faq:
  - q: "When should Android teams adopt play feature delivery on-demand modules?"
    a: "Adopt play feature delivery on-demand modules when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with play feature delivery on-demand modules?"
  ...
  - q: "How do I debug play feature delivery on-demand modules issues in production?"
...
object AndroidPlayFeatureDeliveryOnDemandFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-play-feature-delivery-on-demand_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for play feature delivery on-demand modules
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-play-feature-delivery-on-demand` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get play feature delivery on-demand modules wrong in predictable ways:
...
When play feature delivery on-demand modules misbehaves in production:
...
Before enabling `android-play-feature-delivery-on-demand` for all users:
