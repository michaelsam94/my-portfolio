...
faq:
  - q: "What breaks first with node cluster mode vs worker threads?"
    a: "Misconfigured defaults under load—missing observability, idempotency, or rollback paths."
  - q: "How to test node cluster mode vs worker threads?"
    a: "Integration tests on production-like topology and load at 2× peak."
  - q: "When defer node cluster mode vs worker threads?"
...
When operating **node cluster mode vs worker threads** (`node-cluster-mode-vs-worker-threads`), tie this section to a measurable SLI—latency, error rate, freshness, or throughput—and review it in weekly ops until the pattern is boringly stable.
