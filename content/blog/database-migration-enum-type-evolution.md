---
title: "Database Enum Type Evolution"
slug: "database-migration-enum-type-evolution"
description: "Add enum values safely — commit before use, avoid removing values, migration ordering."
...
tags:
  ...
  - "Backend"
  - "Migrations"
keywords: "database migration enum type evolution, production, backend"
faq:
  - q: "What problem does Database Enum Type Evolution solve?"
    a: "It addresses production gaps teams hit when scaling database migration enum type evolution: correctness under concurrency, operability, and measurable SLOs instead of ad-hoc scripts."
  - q: "When should I adopt this pattern?"
    a: "Adopt when database migration enum type evolution appears on incident timelines, p95 latency regresses, or the next traffic doubling will break the current shortcut."
...
A billing service lost duplicate events because database migration enum type evolution was handled only in application code without database-enforced invariants. The fix was not more logging — it was moving the guarantee to the layer that survives process crashes and duplicate deliveries.

Senior backend work on database enum type evolution is less about syntax and more about failure modes: what happens on retry, on partial outage, and when two deploy versions run simultaneously during a rolling update.
...
Dashboards for database migration enum type evolution should answer: 'Is the system slow, broken, or overloaded?' without SSH. Exemplars link spikes to trace IDs.
...
Measure before optimizing database migration enum type evolution. Capture baseline p50/p95 latency, error rate, and resource utilization under representative load. Change one variable at a time — pool size, batch size, timeout, cache TTL — and re-measure.
...
Ship database migration enum type evolution changes behind feature flags when behavior crosses service boundaries. Use canary deploys with automatic rollback on error rate or latency regression.
