...
description: "AutoMigrationSpec for additive schema changes: export schema, verify migrations, and when manual SQL still wins."
datePublished: "2026-08-04"
dateModified: "2026-08-04"
tags: ["Android", "Room", "Database"]
keywords: "Room auto migration, AutoMigrationSpec, schema export Room Android"
faq:
  - q: "When should Android teams adopt room auto migrations?"
    a: "Adopt room auto migrations when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with room auto migrations?"
  ...
  - q: "How do I debug room auto migrations issues in production?"
...
object AndroidRoomAutoMigrationsFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-room-auto-migrations_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for room auto migrations
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-room-auto-migrations` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get room auto migrations wrong in predictable ways:
...
When room auto migrations misbehaves in production:
...
Before enabling `android-room-auto-migrations` for all users:
