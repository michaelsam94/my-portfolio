---
title: "Material Motion Patterns in Compose"
slug: "android-compose-material-motion-patterns"
description: "Navigation transitions with AnimatedContent and Material easing: hierarchy, elevation, and z-order during motion."
datePublished: "2026-07-24"
dateModified: "2026-07-24"
tags: ["Android", "Jetpack Compose", "UX"]
keywords: "Material motion patterns Compose, navigation animation Material, AnimatedContent navigation"
faq:
  - q: "When should Android teams adopt material motion patterns in compose?"
    a: "Adopt material motion patterns in compose when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with material motion patterns in compose?"
  ...
  - q: "How do I debug material motion patterns in compose issues in production?"
...
object AndroidComposeMaterialMotionPatternsFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-compose-material-motion-patterns_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for material motion patterns in compose
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-compose-material-motion-patterns` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get material motion patterns in compose wrong in predictable ways:
...
When material motion patterns in compose misbehaves in production:
...
Before enabling `android-compose-material-motion-patterns` for all users:
