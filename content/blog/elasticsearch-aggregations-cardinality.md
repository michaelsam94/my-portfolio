---
title: "Elasticsearch Cardinality Aggregations"
slug: "elasticsearch-aggregations-cardinality"
description: "HyperLogLog++ precision — unique counts vs terms agg on high cardinality."
...
tags:
  ...
  - "Backend"
  - "Search"
keywords: "elasticsearch aggregations cardinality, production, backend"
faq:
  - q: "What problem does Elasticsearch Cardinality Aggregations solve?"
    a: "It addresses production gaps teams hit when scaling elasticsearch aggregations cardinality: correctness under concurrency, operability, and measurable SLOs instead of ad-hoc scripts."
  - q: "When should I adopt this pattern?"
    a: "Adopt when elasticsearch aggregations cardinality appears on incident timelines, p95 latency regresses, or the next traffic doubling will break the current shortcut."
...
A billing service lost duplicate events because elasticsearch aggregations cardinality was handled only in application code without database-enforced invariants. The fix was not more logging — it was moving the guarantee to the layer that survives process crashes and duplicate deliveries.

Senior backend work on elasticsearch cardinality aggregations is less about syntax and more about failure modes: what happens on retry, on partial outage, and when two deploy versions run simultaneously during a rolling update.
...
```sql
-- Example: idempotent ingest skeleton for elasticsearch workloads
...
Dashboards for elasticsearch aggregations cardinality should answer: 'Is the system slow, broken, or overloaded?' without SSH. Exemplars link spikes to trace IDs.
...
Measure before optimizing elasticsearch aggregations cardinality. Capture baseline p50/p95 latency, error rate, and resource utilization under representative load. Change one variable at a time — pool size, batch size, timeout, cache TTL — and re-measure.
...
Ship elasticsearch aggregations cardinality changes behind feature flags when behavior crosses service boundaries. Use canary deploys with automatic rollback on error rate or latency regression.
...
Assign an owner for elasticsearch aggregations cardinality standards: code templates, lint rules, and onboarding docs. Platform teams provide paved roads; product teams stay responsible for SLOs.
