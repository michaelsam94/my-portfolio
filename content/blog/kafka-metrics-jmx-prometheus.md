---
title: "Kafka JMX Metrics for Prometheus"
slug: "kafka-metrics-jmx-prometheus"
description: "Export broker and consumer lag metrics — kafka_exporter vs JMX exporter cardinality control."
...
tags:
  ...
  - "Backend"
  - "Distributed Systems"
keywords: "kafka metrics jmx prometheus, production, backend"
faq:
  - q: "What problem does Kafka JMX Metrics solve?"
    a: "It addresses production gaps teams hit when scaling kafka metrics jmx prometheus: correctness under concurrency, operability, and measurable SLOs instead of ad-hoc scripts."
  - q: "When should I adopt this pattern?"
    a: "Adopt when kafka metrics jmx prometheus appears on incident timelines, p95 latency regresses, or the next traffic doubling will break the current shortcut."
...
A billing service lost duplicate events because kafka metrics jmx prometheus was handled only in application code without database-enforced invariants. The fix was not more logging — it was moving the guarantee to the layer that survives process crashes and duplicate deliveries.

Senior backend work on kafka jmx metrics for prometheus is less about syntax and more about failure modes: what happens on retry, on partial outage, and when two deploy versions run simultaneously during a rolling update.
...
Dashboards for kafka metrics jmx prometheus should answer: 'Is the system slow, broken, or overloaded?' without SSH. Exemplars link spikes to trace IDs.
...
Measure before optimizing kafka metrics jmx prometheus. Capture baseline p50/p95 latency, error rate, and resource utilization under representative load. Change one variable at a time — pool size, batch size, timeout, cache TTL — and re-measure.
...
Ship kafka metrics jmx prometheus changes behind feature flags when behavior crosses service boundaries. Use canary deploys with automatic rollback on error rate or latency regression.
