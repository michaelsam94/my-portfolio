---
title: "Always-On Display on Wear Compose"
slug: "android-compose-wear-always-on-display"
description: "Ambient mode lifecycle: burn-in safe layouts, low-bit ambient, and transition between interactive and ambient."
datePublished: "2026-08-01"
dateModified: "2026-08-01"
tags: ["Android", "Wear OS", "Display"]
keywords: "always on display Wear, ambient mode Compose, burn in protection Wear OS"
faq:
  - q: "When should Android teams adopt always-on display on wear compose?"
    a: "Adopt always-on display on wear compose when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with always-on display on wear compose?"
  ...
  - q: "How do I debug always-on display on wear compose issues in production?"
...
object AndroidComposeWearAlwaysOnDisplayFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-compose-wear-always-on-display_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for always-on display on wear compose
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-compose-wear-always-on-display` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get always-on display on wear compose wrong in predictable ways:
...
When always-on display on wear compose misbehaves in production:
...
Before enabling `android-compose-wear-always-on-display` for all users:
