...
description: "Detect VPN, metered, unmetered, and captive portal: download policy, video quality, and sync scheduling."
datePublished: "2026-08-10"
dateModified: "2026-08-10"
tags: ["Android", "Networking", "Offline"]
keywords: "NetworkCapabilities Android, metered network detection, VPN detection Android"
faq:
  - q: "When should Android teams adopt networkcapabilities detection patterns?"
    a: "Adopt networkcapabilities detection patterns when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with networkcapabilities detection patterns?"
  ...
  - q: "How do I debug networkcapabilities detection patterns issues in production?"
...
object AndroidNetworkCapabilitiesDetectionFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-network-capabilities-detection_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for networkcapabilities detection patterns
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-network-capabilities-detection` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get networkcapabilities detection patterns wrong in predictable ways:
...
When networkcapabilities detection patterns misbehaves in production:
...
Before enabling `android-network-capabilities-detection` for all users:
