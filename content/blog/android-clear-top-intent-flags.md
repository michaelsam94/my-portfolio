---
title: "CLEAR_TOP and Intent Flag Combinations"
slug: "android-clear-top-intent-flags"
description: "FLAG_ACTIVITY_CLEAR_TOP with singleTop/singleTask: predictable back stack when navigating from notifications."
datePublished: "2026-07-26"
dateModified: "2026-07-26"
tags: ["Android", "Intents", "Navigation"]
keywords: "FLAG_ACTIVITY_CLEAR_TOP, intent flags Android, clear top navigation"
faq:
  - q: "When should Android teams adopt clear_top and intent flag combinations?"
    a: "Adopt clear_top and intent flag combinations when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with clear_top and intent flag combinations?"
  ...
  - q: "How do I debug clear_top and intent flag combinations issues in production?"
...
object AndroidClearTopIntentFlagsFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-clear-top-intent-flags_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for clear_top and intent flag combinations
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-clear-top-intent-flags` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get clear_top and intent flag combinations wrong in predictable ways:
...
When clear_top and intent flag combinations misbehaves in production:
...
Before enabling `android-clear-top-intent-flags` for all users:
...
## Clear Top Intent Flags on Samsung and Pixel divergence

Exercise clear top intent flags on Galaxy A-series and Pixel a-series — emulators hide OEM battery and storage quirks. Capture Macrobenchmark or Firebase trace for the critical path touching clear; regressions above 8% block release for `android-clear-top-intent-flags`.

Document permission and background behavior in internal runbook: what breaks under Doze, what requires foreground service, and what Play policy declarations apply. Support tickets referencing "Clear Top Intent Flags" should map to a single runbook section with known workarounds.

## Intent regression gates for Play Vitals

Before promoting `android-clear-top-intent-flags` changes past 20% rollout, compare ANR rate, slow cold start, and excessive wakeups against seven-day baseline. Fail rollback review if flags path shows >5% increase in `slow frames` without documented trade-off approval.
