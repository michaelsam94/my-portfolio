...
description: "MediaSessionService with FGS mediaPlayback: notification controls, stopSelf on pause, and Bluetooth disconnect handling."
datePublished: "2026-08-15"
dateModified: "2026-08-15"
tags: ["Android", "Media", "Foreground Service"]
keywords: "media playback foreground service, MediaSessionService Android, media FGS"
faq:
  - q: "When should Android teams adopt media playback foreground service?"
    a: "Adopt media playback foreground service when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with media playback foreground service?"
  ...
  - q: "How do I debug media playback foreground service issues in production?"
...
object AndroidMediaPlaybackForegroundServiceFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-media-playback-foreground-service_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for media playback foreground service
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-media-playback-foreground-service` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get media playback foreground service wrong in predictable ways:
...
When media playback foreground service misbehaves in production:
...
Before enabling `android-media-playback-foreground-service` for all users:
