---
title: "Exposed Dropdown Menu in Compose"
slug: "android-compose-exposed-dropdown-menu"
description: "ExposedDropdownMenuBox: read-only field trigger, menu width, and error state with keyboard dismissal."
datePublished: "2026-08-08"
dateModified: "2026-08-08"
tags: ["Android", "Jetpack Compose", "Material 3"]
keywords: "ExposedDropdownMenu Compose, dropdown menu box Compose, select field Compose"
faq:
  - q: "When should Android teams adopt exposed dropdown menu in compose?"
    a: "Adopt exposed dropdown menu in compose when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with exposed dropdown menu in compose?"
  ...
  - q: "How do I debug exposed dropdown menu in compose issues in production?"
...
object AndroidComposeExposedDropdownMenuFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-compose-exposed-dropdown-menu_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for exposed dropdown menu in compose
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-compose-exposed-dropdown-menu` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get exposed dropdown menu in compose wrong in predictable ways:
...
When exposed dropdown menu in compose misbehaves in production:
...
Before enabling `android-compose-exposed-dropdown-menu` for all users:
