---
title: "Screen Capture Prevention on Android"
slug: "android-screen-capture-prevention"
description: "FLAG_SECURE for sensitive screens: compliance trade-offs, screenshot detection API, and screen recording on Android 14+."
datePublished: "2026-08-07"
dateModified: "2026-08-07"
tags: ["Android", "Security", "Compliance"]
keywords: "FLAG_SECURE Android, screen capture prevention, screenshot detection API"
faq:
  - q: "When should Android teams adopt screen capture prevention on android?"
    a: "Adopt screen capture prevention on android when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with screen capture prevention on android?"
  ...
  - q: "How do I debug screen capture prevention on android issues in production?"
...
object AndroidScreenCapturePreventionFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-screen-capture-prevention_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for screen capture prevention on android
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-screen-capture-prevention` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get screen capture prevention on android wrong in predictable ways:
...
When screen capture prevention on android misbehaves in production:
...
Before enabling `android-screen-capture-prevention` for all users:
