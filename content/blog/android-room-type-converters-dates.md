---
title: "Room Type Converters for Dates and Enums"
slug: "android-room-type-converters-dates"
description: "Type converters for Room: Instant, LocalDate, enums, and JSON columns — migration-safe patterns and testing converter round-trips."
datePublished: "2026-07-19"
dateModified: "2026-07-19"
tags: ["Android", "Room", "Kotlin"]
keywords: "Room type converters, Instant Room, enum converter, Room custom types"
faq:
  - q: "When should Android teams adopt room type converters for dates and enums?"
    a: "Adopt room type converters for dates and enums when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with room type converters for dates and enums?"
  ...
  - q: "How do I debug room type converters for dates and enums issues in production?"
...
object AndroidRoomTypeConvertersDatesFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-room-type-converters-dates_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for room type converters for dates and enums
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-room-type-converters-dates` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get room type converters for dates and enums wrong in predictable ways:
...
When room type converters for dates and enums misbehaves in production:
...
Before enabling `android-room-type-converters-dates` for all users:
