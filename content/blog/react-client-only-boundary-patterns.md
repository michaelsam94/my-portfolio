---
title: "Client-Only Boundary Patterns"
slug: "react-client-only-boundary-patterns"
description: "Dynamic import with ssr:false and useEffect gates — third-party widgets and browser-only APIs safely."
datePublished: "2026-12-27"
dateModified: "2026-12-27"
tags: ["React", "SSR", "Hydration"]
keywords: "client only React, ssr false dynamic import, browser only component"
faq:
  - q: "What is Client-Only Boundary Patterns?"
    a: "Client-Only Boundary Patterns is a production pattern for frontend and product engineering teams building performant, accessible web applications. It addresses real constraints around user experience, security, and measurable outcomes — not theoretical best practices disconnected from shipping code."
  - q: "When should teams adopt Client-Only Boundary Patterns?"
    a: "Adopt Client-Only Boundary Patterns when you have field data or user research showing pain — slow interactions, accessibility gaps, conversion drop-offs, or security findings — and simpler fixes have been exhausted. Pilot on one route or feature before rolling out platform-wide."
  - q: "What are common mistakes with Client-Only Boundary Patterns?"
...
Document which metrics you expect to move. If client-only boundary patterns is a performance change, baseline LCP, INP, and CLS in CrUX or your RUM tool for affected routes before merging. If it is an accessibility change, run axe and manual screen reader checks on the critical path — not just the component story.

## Implementation patterns

Start with the smallest change that proves the approach. For client-only boundary patterns, that usually means one route, one component tree, or one middleware rule — not a platform-wide migration.

```tsx
// Example: progressive adoption pattern
// Step 1 — isolate behind a feature flag or route segment
export async function Page() {
  const enabled = await flags.isEnabled("react_client_only_boundary_patterns");
...
Teams get client-only boundary patterns wrong in predictable ways:
...
When client-only boundary patterns misbehaves in production, work top-down:
