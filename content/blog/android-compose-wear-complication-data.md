---
title: "Wear Complication Data in Compose"
slug: "android-compose-wear-complication-data"
description: "ComplicationDataSourceService with Compose previews: timeline entries, short text complications, and updates."
datePublished: "2026-07-27"
dateModified: "2026-07-27"
tags: ["Android", "Wear OS", "Compose"]
keywords: "Wear complication Compose, ComplicationDataSourceService, watch face complication data"
faq:
  - q: "When should Android teams adopt wear complication data in compose?"
    a: "Adopt wear complication data in compose when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with wear complication data in compose?"
  ...
  - q: "How do I debug wear complication data in compose issues in production?"
...
object AndroidComposeWearComplicationDataFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-compose-wear-complication-data_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for wear complication data in compose
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-compose-wear-complication-data` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get wear complication data in compose wrong in predictable ways:
...
When wear complication data in compose misbehaves in production:
...
Before enabling `android-compose-wear-complication-data` for all users:
