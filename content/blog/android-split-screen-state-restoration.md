---
title: "Split-Screen State Restoration"
slug: "android-split-screen-state-restoration"
description: "Save state across multi-window resize: ViewModel retention, configChanges, and Compose saveable in split mode."
datePublished: "2026-07-22"
dateModified: "2026-07-22"
tags: ["Android", "Multi-Window", "State"]
keywords: "split screen state Android, multi-window state restoration, resize handling"
faq:
  - q: "When should Android teams adopt split-screen state restoration?"
    a: "Adopt split-screen state restoration when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with split-screen state restoration?"
  ...
  - q: "How do I debug split-screen state restoration issues in production?"
...
object AndroidSplitScreenStateRestorationFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-split-screen-state-restoration_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for split-screen state restoration
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-split-screen-state-restoration` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get split-screen state restoration wrong in predictable ways:
...
When split-screen state restoration misbehaves in production:
...
Before enabling `android-split-screen-state-restoration` for all users:
