---
title: "Foreground Service Notification Types"
slug: "android-foreground-service-notification-types"
description: "Map FGS types to user-visible disclosures: dataSync, mediaPlayback, location — and Android 14+ timeout rules."
datePublished: "2026-08-14"
dateModified: "2026-08-14"
tags: ["Android", "Foreground Service", "Notifications"]
keywords: "foreground service types Android, FGS notification, dataSync foreground service"
faq:
  - q: "When should Android teams adopt foreground service notification types?"
    a: "Adopt foreground service notification types when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with foreground service notification types?"
  ...
  - q: "How do I debug foreground service notification types issues in production?"
...
object AndroidForegroundServiceNotificationTypesFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-foreground-service-notification-types_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for foreground service notification types
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-foreground-service-notification-types` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get foreground service notification types wrong in predictable ways:
...
When foreground service notification types misbehaves in production:
...
Before enabling `android-foreground-service-notification-types` for all users:
