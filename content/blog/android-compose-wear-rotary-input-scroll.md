---
title: "Wear OS Rotary Input Scroll in Compose"
slug: "android-compose-wear-rotary-input-scroll"
description: "Rotary scrollable modifiers: focusGroup, rotaryScrollable, and crown input on Wear Compose."
datePublished: "2026-07-26"
dateModified: "2026-07-26"
tags: ["Android", "Wear OS", "Compose"]
keywords: "Wear rotary input Compose, rotaryScrollable Wear, crown scroll Wear OS"
faq:
  - q: "When should Android teams adopt wear os rotary input scroll in compose?"
    a: "Adopt wear os rotary input scroll in compose when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with wear os rotary input scroll in compose?"
  ...
  - q: "How do I debug wear os rotary input scroll in compose issues in production?"
...
object AndroidComposeWearRotaryInputScrollFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-compose-wear-rotary-input-scroll_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for wear os rotary input scroll in compose
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-compose-wear-rotary-input-scroll` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get wear os rotary input scroll in compose wrong in predictable ways:
...
When wear os rotary input scroll in compose misbehaves in production:
...
Before enabling `android-compose-wear-rotary-input-scroll` for all users:
