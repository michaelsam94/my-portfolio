...
slug: "web-performance-main-thread-work"
description: "Break up long tasks — scheduler.yield, requestIdleCallback, and moving work to Web Workers."
...
tags:
  ...
  - "JavaScript"
  - "INP"
keywords: "main thread work reduction, long task splitting, INP optimization"
faq:
  - q: "Main thread work?"
    a: "Parse, JS, style, layout, paint, input — all compete on one thread in typical pages."
  - q: "Worker vs scheduler.yield?"
    a: "Workers for CPU without DOM; yield for chunked DOM-touching updates between frames."
  - q: "Is requestIdleCallback still relevant?"
    a: "Yes for background prefetch and analytics — never for user-visible feedback paths."
faqAnswers:
  - question: "When is web performance main thread work the wrong approach?"
    answer: "When a simpler control already covers the risk, or when the operational cost exceeds the benefit for your threat and traffic model."
  - question: "What should we measure for web performance main thread work?"
    answer: "Pair a leading operational signal with a lagging user or risk outcome, reviewed on a fixed cadence with a named owner."
  - question: "How do we roll back web performance main thread work safely?"
...
## How web performance main thread work works under the hood

Production engineering for web performance main thread work. The mechanism matters because browsers and servers optimize for the common case — not your specific stack. Web Performance Main Thread Work sits at the intersection of user-perceived latency, correctness, and operability.
...
- **The original sin**: Rolling out web performance main thread work without field measurement, rollback, or accessibility checks
...
For web performance main thread work, log correlation IDs across client beacons and server logs. Compare canary vs control during rollout. Roll forward only when p75 field metrics hold for at least one full business day in the target geography.

## What I'd ship this week

We shipped web performance main thread work and discovered the gap between documentation and production the hard way.. If I were prioritizing one action this sprint: pick the single user journey where web performance main thread work hurts most, instrument it, fix the invariant, and only then generalize.
...
Web Performance Main Thread Work rarely lives entirely in the browser or client. Align cache TTLs, API error shapes, and deploy windows with the teams owning those systems — otherwise you optimize one layer while another invalidates gains.
...
When web performance main thread work touches revenue, auth, or compliance, schedule a cross-functional review after major launches. Platform, product, security, and support should agree on the leading metric and rollback owner before wide rollout.
...
**Context:** Web performance main thread work affects users when when web performance main thread work affects users on critical paths. Avoid the failure mode where teams rolling out web performance main thread work without field measurement, rollback, or accessibility checks.
