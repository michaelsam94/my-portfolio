---
title: "Biometric Auth UI in Compose"
slug: "android-compose-biometric-auth-compose"
description: "BiometricPrompt from Compose: CryptoObject unlock flow, error mapping to UI, and fallback PIN navigation."
datePublished: "2026-07-25"
dateModified: "2026-07-25"
tags: ["Android", "Jetpack Compose", "Security"]
keywords: "biometric auth Compose, BiometricPrompt Compose UI, fingerprint login Compose"
faq:
  - q: "When should Android teams adopt biometric auth ui in compose?"
    a: "Adopt biometric auth ui in compose when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with biometric auth ui in compose?"
  ...
  - q: "How do I debug biometric auth ui in compose issues in production?"
...
object AndroidComposeBiometricAuthComposeFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-compose-biometric-auth-compose_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for biometric auth ui in compose
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-compose-biometric-auth-compose` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get biometric auth ui in compose wrong in predictable ways:
...
When biometric auth ui in compose misbehaves in production:
...
Before enabling `android-compose-biometric-auth-compose` for all users:
