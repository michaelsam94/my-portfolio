---
title: "ViewModel Factory with Hilt Assisted Inject"
slug: "android-viewmodel-factory-hilt-assist"
description: "AssistedInject ViewModels: runtime parameters with @HiltViewModel, SavedStateHandle, and navigation args."
datePublished: "2026-07-30"
dateModified: "2026-07-30"
tags: ["Android", "Hilt", "ViewModel"]
keywords: "Hilt assisted inject ViewModel, AssistedInject Android, ViewModel factory Hilt"
faq:
  - q: "When should Android teams adopt viewmodel factory with hilt assisted inject?"
    a: "Adopt viewmodel factory with hilt assisted inject when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with viewmodel factory with hilt assisted inject?"
  ...
  - q: "How do I debug viewmodel factory with hilt assisted inject issues in production?"
...
object AndroidViewmodelFactoryHiltAssistFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-viewmodel-factory-hilt-assist_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for viewmodel factory with hilt assisted inject
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-viewmodel-factory-hilt-assist` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get viewmodel factory with hilt assisted inject wrong in predictable ways:
...
When viewmodel factory with hilt assisted inject misbehaves in production:
...
Before enabling `android-viewmodel-factory-hilt-assist` for all users:
