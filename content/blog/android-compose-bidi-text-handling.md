---
title: "BiDi Text Handling in Compose"
slug: "android-compose-bidi-text-handling"
description: "Mixed LTR/RTL text: Unicode bidi, TextAlign.Start, and email/URL rendering in RTL locales."
datePublished: "2026-07-18"
dateModified: "2026-07-18"
tags: ["Android", "Jetpack Compose", "Localization"]
keywords: "BiDi text Compose, mixed direction text Android, RTL text rendering"
faq:
  - q: "When should Android teams adopt bidi text handling in compose?"
    a: "Adopt bidi text handling in compose when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with bidi text handling in compose?"
  ...
  - q: "How do I debug bidi text handling in compose issues in production?"
...
object AndroidComposeBidiTextHandlingFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-compose-bidi-text-handling_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for bidi text handling in compose
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-compose-bidi-text-handling` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get bidi text handling in compose wrong in predictable ways:
...
When bidi text handling in compose misbehaves in production:
...
Before enabling `android-compose-bidi-text-handling` for all users:
