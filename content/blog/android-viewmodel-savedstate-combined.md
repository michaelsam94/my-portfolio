---
title: "ViewModel Plus SavedStateHandle Combined"
slug: "android-viewmodel-savedstate-combined"
description: "Combine ViewModel business state with SavedStateHandle UI state: scroll positions, form drafts, and tab selection."
datePublished: "2026-07-31"
dateModified: "2026-07-31"
tags: ["Android", "ViewModel", "State"]
keywords: "SavedStateHandle ViewModel, ViewModel UI state, form draft SavedStateHandle"
faq:
  - q: "When should Android teams adopt viewmodel plus savedstatehandle combined?"
    a: "Adopt viewmodel plus savedstatehandle combined when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with viewmodel plus savedstatehandle combined?"
  ...
  - q: "How do I debug viewmodel plus savedstatehandle combined issues in production?"
...
object AndroidViewmodelSavedstateCombinedFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-viewmodel-savedstate-combined_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for viewmodel plus savedstatehandle combined
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-viewmodel-savedstate-combined` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get viewmodel plus savedstatehandle combined wrong in predictable ways:
...
When viewmodel plus savedstatehandle combined misbehaves in production:
...
Before enabling `android-viewmodel-savedstate-combined` for all users:
