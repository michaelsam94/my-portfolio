---
title: "AlarmManager Exact vs Inexact Alarms"
slug: "android-alarmmanager-exact-vs-inexact"
description: "Schedule alarms correctly: SCHEDULE_EXACT_ALARM permission, inexact batching for battery, and migrating from setRepeating."
datePublished: "2026-08-09"
dateModified: "2026-08-09"
tags: ["Android", "AlarmManager", "Background"]
keywords: "AlarmManager exact alarm, inexact alarm Android, SCHEDULE_EXACT_ALARM permission"
faq:
  - q: "When should Android teams adopt alarmmanager exact vs inexact alarms?"
    a: "Adopt alarmmanager exact vs inexact alarms when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with alarmmanager exact vs inexact alarms?"
  ...
  - q: "How do I debug alarmmanager exact vs inexact alarms issues in production?"
...
object AndroidAlarmmanagerExactVsInexactFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-alarmmanager-exact-vs-inexact_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for alarmmanager exact vs inexact alarms
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-alarmmanager-exact-vs-inexact` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get alarmmanager exact vs inexact alarms wrong in predictable ways:
...
When alarmmanager exact vs inexact alarms misbehaves in production:
...
Before enabling `android-alarmmanager-exact-vs-inexact` for all users:
...
## Alarmmanager Exact Vs Inexact on Samsung and Pixel divergence

Exercise alarmmanager exact vs inexact on Galaxy A-series and Pixel a-series — emulators hide OEM battery and storage quirks. Capture Macrobenchmark or Firebase trace for the critical path touching alarmmanager; regressions above 8% block release for `android-alarmmanager-exact-vs-inexact`.

Document permission and background behavior in internal runbook: what breaks under Doze, what requires foreground service, and what Play policy declarations apply. Support tickets referencing "Alarmmanager Exact Vs Inexact" should map to a single runbook section with known workarounds.

## Vs regression gates for Play Vitals

Before promoting `android-alarmmanager-exact-vs-inexact` changes past 20% rollout, compare ANR rate, slow cold start, and excessive wakeups against seven-day baseline. Fail rollback review if inexact path shows >5% increase in `slow frames` without documented trade-off approval.
