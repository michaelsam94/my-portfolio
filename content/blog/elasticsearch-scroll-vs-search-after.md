---
title: "Elasticsearch Scroll vs search_after"
slug: "elasticsearch-scroll-vs-search-after"
description: "Deep pagination — avoid scroll in user-facing APIs, use PIT + search_after."
...
tags:
  ...
  - "Backend"
  - "Search"
keywords: "elasticsearch scroll vs search after, production, backend"
faq:
  - q: "What problem does Elasticsearch Scroll vs search_after solve?"
    a: "It addresses production gaps teams hit when scaling elasticsearch scroll vs search after: correctness under concurrency, operability, and measurable SLOs instead of ad-hoc scripts."
  - q: "When should I adopt this pattern?"
    a: "Adopt when elasticsearch scroll vs search after appears on incident timelines, p95 latency regresses, or the next traffic doubling will break the current shortcut."
...
A billing service lost duplicate events because elasticsearch scroll vs search after was handled only in application code without database-enforced invariants. The fix was not more logging — it was moving the guarantee to the layer that survives process crashes and duplicate deliveries.

Senior backend work on elasticsearch scroll vs search_after is less about syntax and more about failure modes: what happens on retry, on partial outage, and when two deploy versions run simultaneously during a rolling update.
...
Dashboards for elasticsearch scroll vs search after should answer: 'Is the system slow, broken, or overloaded?' without SSH. Exemplars link spikes to trace IDs.
...
Measure before optimizing elasticsearch scroll vs search after. Capture baseline p50/p95 latency, error rate, and resource utilization under representative load. Change one variable at a time — pool size, batch size, timeout, cache TTL — and re-measure.
...
Ship elasticsearch scroll vs search after changes behind feature flags when behavior crosses service boundaries. Use canary deploys with automatic rollback on error rate or latency regression.
