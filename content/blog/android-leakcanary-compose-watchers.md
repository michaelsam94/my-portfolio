---
title: "LeakCanary Watchers for Compose"
slug: "android-leakcanary-compose-watchers"
description: "Detect Compose-related leaks: rememberCoroutineScope, NavController, and custom RetainedObjectWatcher hooks."
datePublished: "2026-07-19"
dateModified: "2026-07-19"
tags: ["Android", "Performance", "LeakCanary"]
keywords: "LeakCanary Compose, memory leak Compose Android, LeakCanary watchers"
faq:
  - q: "When should Android teams adopt leakcanary watchers for compose?"
    a: "Adopt leakcanary watchers for compose when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with leakcanary watchers for compose?"
  ...
  - q: "How do I debug leakcanary watchers for compose issues in production?"
...
object AndroidLeakcanaryComposeWatchersFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-leakcanary-compose-watchers_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for leakcanary watchers for compose
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-leakcanary-compose-watchers` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get leakcanary watchers for compose wrong in predictable ways:
...
When leakcanary watchers for compose misbehaves in production:
...
Before enabling `android-leakcanary-compose-watchers` for all users:
