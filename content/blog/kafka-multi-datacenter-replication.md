---
title: "Kafka Multi-Datacenter Replication"
slug: "kafka-multi-datacenter-replication"
description: "Active-active vs hub-spoke replication — offset sync, conflict resolution, and network partition behavior."
...
tags:
  ...
  - "Backend"
  - "Distributed Systems"
keywords: "kafka multi datacenter replication, production, backend"
faq:
  - q: "What problem does Kafka Multi-Datacenter Replication solve?"
    a: "It addresses production gaps teams hit when scaling kafka multi datacenter replication: correctness under concurrency, operability, and measurable SLOs instead of ad-hoc scripts."
  - q: "When should I adopt this pattern?"
    a: "Adopt when kafka multi datacenter replication appears on incident timelines, p95 latency regresses, or the next traffic doubling will break the current shortcut."
...
A billing service lost duplicate events because kafka multi datacenter replication was handled only in application code without database-enforced invariants. The fix was not more logging — it was moving the guarantee to the layer that survives process crashes and duplicate deliveries.

Senior backend work on kafka multi-datacenter replication is less about syntax and more about failure modes: what happens on retry, on partial outage, and when two deploy versions run simultaneously during a rolling update.
...
Dashboards for kafka multi datacenter replication should answer: 'Is the system slow, broken, or overloaded?' without SSH. Exemplars link spikes to trace IDs.
...
Measure before optimizing kafka multi datacenter replication. Capture baseline p50/p95 latency, error rate, and resource utilization under representative load. Change one variable at a time — pool size, batch size, timeout, cache TTL — and re-measure.
...
Ship kafka multi datacenter replication changes behind feature flags when behavior crosses service boundaries. Use canary deploys with automatic rollback on error rate or latency regression.
