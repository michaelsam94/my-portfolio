---
title: "Modal Bottom Sheet in Compose"
slug: "android-compose-modal-bottom-sheet"
description: "ModalBottomSheet with sheet state: partial expand, confirm dismiss, and nested scroll connection for lists."
datePublished: "2026-08-09"
dateModified: "2026-08-09"
tags: ["Android", "Jetpack Compose", "Material 3"]
keywords: "ModalBottomSheet Compose, bottom sheet state Compose, partial expand sheet"
faq:
  - q: "When should Android teams adopt modal bottom sheet in compose?"
    a: "Adopt modal bottom sheet in compose when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with modal bottom sheet in compose?"
  ...
  - q: "How do I debug modal bottom sheet in compose issues in production?"
...
object AndroidComposeModalBottomSheetFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-compose-modal-bottom-sheet_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for modal bottom sheet in compose
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-compose-modal-bottom-sheet` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get modal bottom sheet in compose wrong in predictable ways:
...
When modal bottom sheet in compose misbehaves in production:
...
Before enabling `android-compose-modal-bottom-sheet` for all users:
