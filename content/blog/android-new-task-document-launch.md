---
title: "documentLaunchMode and New Task Patterns"
slug: "android-new-task-document-launch"
description: "Into existing document, always, never: multi-document apps, Chrome-like tabs, and recent tasks entries."
datePublished: "2026-07-27"
dateModified: "2026-07-27"
tags: ["Android", "Activities", "Multi-Window"]
keywords: "documentLaunchMode Android, new document launch, multi document Android app"
faq:
  - q: "When should Android teams adopt documentlaunchmode and new task patterns?"
    a: "Adopt documentlaunchmode and new task patterns when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with documentlaunchmode and new task patterns?"
  ...
  - q: "How do I debug documentlaunchmode and new task patterns issues in production?"
...
object AndroidNewTaskDocumentLaunchFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-new-task-document-launch_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for documentlaunchmode and new task patterns
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-new-task-document-launch` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get documentlaunchmode and new task patterns wrong in predictable ways:
...
When documentlaunchmode and new task patterns misbehaves in production:
...
Before enabling `android-new-task-document-launch` for all users:
