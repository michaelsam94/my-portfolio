---
title: "Collapsing Toolbar with LazyColumn"
slug: "android-compose-collapsing-toolbar-lazy"
description: "Large top app bar collapse: offset tracking, parallax header, and pinned tabs below collapsing region."
datePublished: "2026-08-14"
dateModified: "2026-08-14"
tags: ["Android", "Jetpack Compose", "UI"]
keywords: "collapsing toolbar Compose, large top app bar LazyColumn, parallax header Compose"
faq:
  - q: "When should Android teams adopt collapsing toolbar with lazycolumn?"
    a: "Adopt collapsing toolbar with lazycolumn when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with collapsing toolbar with lazycolumn?"
  ...
  - q: "How do I debug collapsing toolbar with lazycolumn issues in production?"
...
object AndroidComposeCollapsingToolbarLazyFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-compose-collapsing-toolbar-lazy_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for collapsing toolbar with lazycolumn
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-compose-collapsing-toolbar-lazy` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get collapsing toolbar with lazycolumn wrong in predictable ways:
...
When collapsing toolbar with lazycolumn misbehaves in production:
...
Before enabling `android-compose-collapsing-toolbar-lazy` for all users:
