---
title: "Database Flyway Baseline for Legacy DB"
slug: "database-migration-flyway-baseline"
description: "Baseline existing schema — version table without replaying historical DDL on brownfield."
...
tags:
  ...
  - "Backend"
  - "Migrations"
keywords: "database migration flyway baseline, production, backend"
faq:
  - q: "What problem does Database Flyway Baseline solve?"
    a: "It addresses production gaps teams hit when scaling database migration flyway baseline: correctness under concurrency, operability, and measurable SLOs instead of ad-hoc scripts."
  - q: "When should I adopt this pattern?"
    a: "Adopt when database migration flyway baseline appears on incident timelines, p95 latency regresses, or the next traffic doubling will break the current shortcut."
...
A billing service lost duplicate events because database migration flyway baseline was handled only in application code without database-enforced invariants. The fix was not more logging — it was moving the guarantee to the layer that survives process crashes and duplicate deliveries.

Senior backend work on database flyway baseline for legacy db is less about syntax and more about failure modes: what happens on retry, on partial outage, and when two deploy versions run simultaneously during a rolling update.
...
Dashboards for database migration flyway baseline should answer: 'Is the system slow, broken, or overloaded?' without SSH. Exemplars link spikes to trace IDs.
...
Measure before optimizing database migration flyway baseline. Capture baseline p50/p95 latency, error rate, and resource utilization under representative load. Change one variable at a time — pool size, batch size, timeout, cache TTL — and re-measure.
...
Ship database migration flyway baseline changes behind feature flags when behavior crosses service boundaries. Use canary deploys with automatic rollback on error rate or latency regression.
