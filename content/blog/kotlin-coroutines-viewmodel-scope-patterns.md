---
title: "viewModelScope Coroutine Patterns"
slug: "kotlin-coroutines-viewmodel-scope-patterns"
description: "Structured concurrency in ViewModels: supervisorJob, async parallel loads, and cancellation on navigate away."
datePublished: "2026-07-23"
dateModified: "2026-07-23"
tags: ["Kotlin", "Coroutines", "ViewModel"]
keywords: "viewModelScope patterns, ViewModel coroutine Android, structured concurrency ViewModel"
faq:
  - q: "When should Android teams adopt viewmodelscope coroutine patterns?"
    a: "Adopt viewmodelscope coroutine patterns when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with viewmodelscope coroutine patterns?"
  ...
  - q: "How do I debug viewmodelscope coroutine patterns issues in production?"
...
object KotlinCoroutinesViewmodelScopePatternsFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("kotlin-coroutines-viewmodel-scope-patterns_enabled", default = false)
}
...
class KotlinRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for viewmodelscope coroutine patterns
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `kotlin-coroutines-viewmodel-scope-patterns` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get viewmodelscope coroutine patterns wrong in predictable ways:
...
When viewmodelscope coroutine patterns misbehaves in production:
...
Before enabling `kotlin-coroutines-viewmodel-scope-patterns` for all users:
