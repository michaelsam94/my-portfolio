---
title: "Extended FAB and Speed Dial Menu"
slug: "android-compose-extended-fab-menu"
description: "Animated FAB expansion, speed dial accessibility, and replacing deprecated speed dial with explicit menus."
datePublished: "2026-07-31"
dateModified: "2026-07-31"
tags: ["Android", "Jetpack Compose", "Material 3"]
keywords: "extended FAB Compose, speed dial menu Compose, FAB menu animation"
faq:
  - q: "When should Android teams adopt extended fab and speed dial menu?"
    a: "Adopt extended fab and speed dial menu when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with extended fab and speed dial menu?"
  ...
  - q: "How do I debug extended fab and speed dial menu issues in production?"
...
object AndroidComposeExtendedFabMenuFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-compose-extended-fab-menu_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for extended fab and speed dial menu
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-compose-extended-fab-menu` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get extended fab and speed dial menu wrong in predictable ways:
...
When extended fab and speed dial menu misbehaves in production:
...
Before enabling `android-compose-extended-fab-menu` for all users:
