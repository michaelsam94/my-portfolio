---
title: "Suspend Callback Adapter for Legacy APIs"
slug: "kotlin-suspend-callback-adapter"
description: "Wrap Callback/Future APIs with suspendCoroutine and cancellable continuations — Retrofit, Firebase, GMS."
datePublished: "2026-07-24"
dateModified: "2026-07-24"
tags: ["Kotlin", "Coroutines", "Interop"]
keywords: "suspendCoroutine Android, callback to suspend Kotlin, cancellable continuation legacy API"
faq:
  - q: "When should Android teams adopt suspend callback adapter for legacy apis?"
    a: "Adopt suspend callback adapter for legacy apis when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with suspend callback adapter for legacy apis?"
  ...
  - q: "How do I debug suspend callback adapter for legacy apis issues in production?"
...
object KotlinSuspendCallbackAdapterFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("kotlin-suspend-callback-adapter_enabled", default = false)
}
...
class KotlinRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for suspend callback adapter for legacy apis
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `kotlin-suspend-callback-adapter` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get suspend callback adapter for legacy apis wrong in predictable ways:
...
When suspend callback adapter for legacy apis misbehaves in production:
...
Before enabling `kotlin-suspend-callback-adapter` for all users:
