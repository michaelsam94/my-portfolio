---
title: "Clipboard Sensitive Data Handling"
slug: "android-clipboard-sensitive-data"
description: "Clear clipboard on background, disable copy on OTP fields, and Android 13+ clipboard overlay notifications."
datePublished: "2026-07-18"
dateModified: "2026-07-18"
tags: ["Android", "Security", "UX"]
keywords: "clipboard security Android, sensitive data clipboard, clipboard overlay Android 13"
faq:
  - q: "When should Android teams adopt clipboard sensitive data handling?"
    a: "Adopt clipboard sensitive data handling when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with clipboard sensitive data handling?"
  ...
  - q: "How do I debug clipboard sensitive data handling issues in production?"
...
object AndroidClipboardSensitiveDataFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-clipboard-sensitive-data_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for clipboard sensitive data handling
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-clipboard-sensitive-data` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get clipboard sensitive data handling wrong in predictable ways:
...
When clipboard sensitive data handling misbehaves in production:
...
Before enabling `android-clipboard-sensitive-data` for all users:
...
## Clipboard Sensitive Data on Samsung and Pixel divergence

Exercise clipboard sensitive data on Galaxy A-series and Pixel a-series — emulators hide OEM battery and storage quirks. Capture Macrobenchmark or Firebase trace for the critical path touching clipboard; regressions above 8% block release for `android-clipboard-sensitive-data`.

Document permission and background behavior in internal runbook: what breaks under Doze, what requires foreground service, and what Play policy declarations apply. Support tickets referencing "Clipboard Sensitive Data" should map to a single runbook section with known workarounds.

## Sensitive regression gates for Play Vitals

Before promoting `android-clipboard-sensitive-data` changes past 20% rollout, compare ANR rate, slow cold start, and excessive wakeups against seven-day baseline. Fail rollback review if data path shows >5% increase in `slow frames` without documented trade-off approval.
