---
title: "Espresso and Compose Test Interop"
slug: "android-espresso-compose-interop"
description: "composeTestRule + Espresso on same screen: idling synchronizing, hybrid View/Compose hierarchies in tests."
datePublished: "2026-08-14"
dateModified: "2026-08-14"
tags: ["Android", "Testing", "Espresso"]
keywords: "Espresso Compose interop test, hybrid UI test Android, Compose Espresso same screen"
faq:
  - q: "When should Android teams adopt espresso and compose test interop?"
    a: "Adopt espresso and compose test interop when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with espresso and compose test interop?"
  ...
  - q: "How do I debug espresso and compose test interop issues in production?"
...
object AndroidEspressoComposeInteropFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-espresso-compose-interop_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for espresso and compose test interop
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-espresso-compose-interop` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get espresso and compose test interop wrong in predictable ways:
...
When espresso and compose test interop misbehaves in production:
...
Before enabling `android-espresso-compose-interop` for all users:
...
## Espresso Compose Interop on Samsung and Pixel divergence

Exercise espresso compose interop on Galaxy A-series and Pixel a-series — emulators hide OEM battery and storage quirks. Capture Macrobenchmark or Firebase trace for the critical path touching espresso; regressions above 8% block release for `android-espresso-compose-interop`.

Document permission and background behavior in internal runbook: what breaks under Doze, what requires foreground service, and what Play policy declarations apply. Support tickets referencing "Espresso Compose Interop" should map to a single runbook section with known workarounds.

## Compose regression gates for Play Vitals

Before promoting `android-espresso-compose-interop` changes past 20% rollout, compare ANR rate, slow cold start, and excessive wakeups against seven-day baseline. Fail rollback review if interop path shows >5% increase in `slow frames` without documented trade-off approval.
