...
description: "Android 12+ trampoline limits: start activities from BroadcastReceiver, Activity PendingIntent patterns, and BAL exceptions."
datePublished: "2026-08-13"
dateModified: "2026-08-13"
tags: ["Android", "Notifications", "Security"]
keywords: "notification trampoline Android, BAL restrictions, PendingIntent activity launch"
faq:
  - q: "When should Android teams adopt notification trampoline restrictions?"
    a: "Adopt notification trampoline restrictions when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with notification trampoline restrictions?"
  ...
  - q: "How do I debug notification trampoline restrictions issues in production?"
...
object AndroidNotificationTrampolineRestrictionsFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-notification-trampoline-restrictions_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for notification trampoline restrictions
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-notification-trampoline-restrictions` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get notification trampoline restrictions wrong in predictable ways:
...
When notification trampoline restrictions misbehaves in production:
...
Before enabling `android-notification-trampoline-restrictions` for all users:
