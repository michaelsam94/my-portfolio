---
title: "Suggestion and Input Chips in Compose"
slug: "android-compose-suggestion-chip-input"
description: "AssistChip, SuggestionChip, InputChip: search suggestions, tag entry, and keyboard submit handling."
datePublished: "2026-08-07"
dateModified: "2026-08-07"
tags: ["Android", "Jetpack Compose", "Material 3"]
keywords: "InputChip Compose, SuggestionChip, chip input field Compose"
faq:
  - q: "When should Android teams adopt suggestion and input chips in compose?"
    a: "Adopt suggestion and input chips in compose when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with suggestion and input chips in compose?"
  ...
  - q: "How do I debug suggestion and input chips in compose issues in production?"
...
object AndroidComposeSuggestionChipInputFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-compose-suggestion-chip-input_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for suggestion and input chips in compose
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-compose-suggestion-chip-input` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get suggestion and input chips in compose wrong in predictable ways:
...
When suggestion and input chips in compose misbehaves in production:
...
Before enabling `android-compose-suggestion-chip-input` for all users:
