---
title: "gRPC Keepalive and Idle Timeout"
slug: "grpc-keepalive-idle-timeout"
description: "HTTP/2 PING settings — detect dead connections behind NAT without killing idle streams."
...
tags:
  ...
  - "Backend"
  - "API"
keywords: "grpc keepalive idle timeout, production, backend"
faq:
  - q: "What problem does gRPC Keepalive and Idle Timeout solve?"
    a: "It addresses production gaps teams hit when scaling grpc keepalive idle timeout: correctness under concurrency, operability, and measurable SLOs instead of ad-hoc scripts."
  - q: "When should I adopt this pattern?"
    a: "Adopt when grpc keepalive idle timeout appears on incident timelines, p95 latency regresses, or the next traffic doubling will break the current shortcut."
...
A billing service lost duplicate events because grpc keepalive idle timeout was handled only in application code without database-enforced invariants. The fix was not more logging — it was moving the guarantee to the layer that survives process crashes and duplicate deliveries.

Senior backend work on grpc keepalive and idle timeout is less about syntax and more about failure modes: what happens on retry, on partial outage, and when two deploy versions run simultaneously during a rolling update.
...
Dashboards for grpc keepalive idle timeout should answer: 'Is the system slow, broken, or overloaded?' without SSH. Exemplars link spikes to trace IDs.
...
Measure before optimizing grpc keepalive idle timeout. Capture baseline p50/p95 latency, error rate, and resource utilization under representative load. Change one variable at a time — pool size, batch size, timeout, cache TTL — and re-measure.
...
Ship grpc keepalive idle timeout changes behind feature flags when behavior crosses service boundaries. Use canary deploys with automatic rollback on error rate or latency regression.
