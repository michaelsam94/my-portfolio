---
title: "Bottom App Bar with FAB Cutout"
slug: "android-compose-bottom-app-bar-fab"
description: "BottomAppBar with cutout FAB: scroll behavior, snackbar anchor, and Material 3 bottom bar migration."
datePublished: "2026-07-25"
dateModified: "2026-07-25"
tags: ["Android", "Jetpack Compose", "Material 3"]
keywords: "BottomAppBar Compose, FAB cutout Compose, bottom bar scroll behavior"
faq:
  - q: "When should Android teams adopt bottom app bar with fab cutout?"
    a: "Adopt bottom app bar with fab cutout when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with bottom app bar with fab cutout?"
  ...
  - q: "How do I debug bottom app bar with fab cutout issues in production?"
...
object AndroidComposeBottomAppBarFabFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-compose-bottom-app-bar-fab_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for bottom app bar with fab cutout
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-compose-bottom-app-bar-fab` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get bottom app bar with fab cutout wrong in predictable ways:
...
When bottom app bar with fab cutout misbehaves in production:
...
Before enabling `android-compose-bottom-app-bar-fab` for all users:
