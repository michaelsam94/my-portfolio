---
title: "SnackbarHostState Patterns in Compose"
slug: "android-compose-snackbar-host-state"
description: "Show snackbar from ViewModel events: Scaffold host state, action handling, and queue vs replace policy."
datePublished: "2026-07-23"
dateModified: "2026-07-23"
tags: ["Android", "Jetpack Compose", "UX"]
keywords: "SnackbarHostState Compose, snackbar from ViewModel, Scaffold snackbar Compose"
faq:
  - q: "When should Android teams adopt snackbarhoststate patterns in compose?"
    a: "Adopt snackbarhoststate patterns in compose when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with snackbarhoststate patterns in compose?"
  ...
  - q: "How do I debug snackbarhoststate patterns in compose issues in production?"
...
object AndroidComposeSnackbarHostStateFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-compose-snackbar-host-state_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for snackbarhoststate patterns in compose
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-compose-snackbar-host-state` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get snackbarhoststate patterns in compose wrong in predictable ways:
...
When snackbarhoststate patterns in compose misbehaves in production:
...
Before enabling `android-compose-snackbar-host-state` for all users:
