...
description: "CoroutineWorker vs Worker: suspend doWork, foreground info, progress reporting, and cancellation handling."
datePublished: "2026-08-08"
dateModified: "2026-08-08"
tags: ["Android", "WorkManager", "Coroutines"]
keywords: "CoroutineWorker Android, suspend doWork WorkManager, worker progress reporting"
faq:
  - q: "When should Android teams adopt coroutineworker best practices?"
    a: "Adopt coroutineworker best practices when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with coroutineworker best practices?"
  ...
  - q: "How do I debug coroutineworker best practices issues in production?"
...
object AndroidWorkmanagerCoroutineWorkerFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-workmanager-coroutine-worker_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for coroutineworker best practices
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-workmanager-coroutine-worker` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get coroutineworker best practices wrong in predictable ways:
...
When coroutineworker best practices misbehaves in production:
...
Before enabling `android-workmanager-coroutine-worker` for all users:
