---
title: "Intent Redirection Vulnerabilities"
slug: "android-intent-redirection-vulnerabilities"
description: "Audit exported components for intent redirection: validate caller, strip sensitive extras, and use PendingIntent flags."
datePublished: "2026-08-10"
dateModified: "2026-08-10"
tags: ["Android", "Security", "Intents"]
keywords: "intent redirection Android, exported component security, PendingIntent security"
faq:
  - q: "When should Android teams adopt intent redirection vulnerabilities?"
    a: "Adopt intent redirection vulnerabilities when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with intent redirection vulnerabilities?"
  ...
  - q: "How do I debug intent redirection vulnerabilities issues in production?"
...
object AndroidIntentRedirectionVulnerabilitiesFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-intent-redirection-vulnerabilities_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for intent redirection vulnerabilities
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-intent-redirection-vulnerabilities` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get intent redirection vulnerabilities wrong in predictable ways:
...
When intent redirection vulnerabilities misbehaves in production:
...
Before enabling `android-intent-redirection-vulnerabilities` for all users:
