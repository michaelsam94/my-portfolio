---
title: "Kafka Cooperative Sticky Rebalance"
slug: "kafka-rebalance-cooperative-sticky"
description: "Upgrade consumer groups to cooperative-sticky assignor to reduce stop-the-world rebalances."
...
tags:
  ...
  - "Backend"
  - "Distributed Systems"
keywords: "kafka rebalance cooperative sticky, production, backend"
faq:
  - q: "What problem does Kafka Cooperative Sticky Rebalance solve?"
    a: "It addresses production gaps teams hit when scaling kafka rebalance cooperative sticky: correctness under concurrency, operability, and measurable SLOs instead of ad-hoc scripts."
  - q: "When should I adopt this pattern?"
    a: "Adopt when kafka rebalance cooperative sticky appears on incident timelines, p95 latency regresses, or the next traffic doubling will break the current shortcut."
...
A billing service lost duplicate events because kafka rebalance cooperative sticky was handled only in application code without database-enforced invariants. The fix was not more logging — it was moving the guarantee to the layer that survives process crashes and duplicate deliveries.

Senior backend work on kafka cooperative sticky rebalance is less about syntax and more about failure modes: what happens on retry, on partial outage, and when two deploy versions run simultaneously during a rolling update.
...
Dashboards for kafka rebalance cooperative sticky should answer: 'Is the system slow, broken, or overloaded?' without SSH. Exemplars link spikes to trace IDs.
...
Measure before optimizing kafka rebalance cooperative sticky. Capture baseline p50/p95 latency, error rate, and resource utilization under representative load. Change one variable at a time — pool size, batch size, timeout, cache TTL — and re-measure.
...
Ship kafka rebalance cooperative sticky changes behind feature flags when behavior crosses service boundaries. Use canary deploys with automatic rollback on error rate or latency regression.
