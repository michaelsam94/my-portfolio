---
title: "Activity Result Contracts in Compose"
slug: "android-compose-activity-result-contracts"
description: "rememberLauncherForActivityResult: photo picker, permissions, and document contracts with state restoration."
datePublished: "2026-07-23"
dateModified: "2026-07-23"
tags: ["Android", "Jetpack Compose", "Activity Result"]
keywords: "rememberLauncherForActivityResult Compose, activity result Compose, photo picker Compose"
faq:
  - q: "When should Android teams adopt activity result contracts in compose?"
    a: "Adopt activity result contracts in compose when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with activity result contracts in compose?"
  ...
  - q: "How do I debug activity result contracts in compose issues in production?"
...
object AndroidComposeActivityResultContractsFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-compose-activity-result-contracts_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for activity result contracts in compose
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-compose-activity-result-contracts` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get activity result contracts in compose wrong in predictable ways:
...
When activity result contracts in compose misbehaves in production:
...
Before enabling `android-compose-activity-result-contracts` for all users:
