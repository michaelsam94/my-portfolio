---
title: "Health Services on Wear Compose"
slug: "android-compose-wear-health-services"
description: "ExerciseClient, PassiveMonitoringClient: permissions, battery impact, and foreground service on Wear."
datePublished: "2026-07-31"
dateModified: "2026-07-31"
tags: ["Android", "Wear OS", "Health"]
keywords: "Health Services Wear OS, ExerciseClient, passive monitoring watch Android"
faq:
  - q: "When should Android teams adopt health services on wear compose?"
    a: "Adopt health services on wear compose when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with health services on wear compose?"
  ...
  - q: "How do I debug health services on wear compose issues in production?"
...
object AndroidComposeWearHealthServicesFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-compose-wear-health-services_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for health services on wear compose
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-compose-wear-health-services` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get health services on wear compose wrong in predictable ways:
...
When health services on wear compose misbehaves in production:
...
Before enabling `android-compose-wear-health-services` for all users:
