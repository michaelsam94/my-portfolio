---
title: "Postgres Sequence Gap and Contention"
slug: "postgres-sequence-gap-contention"
description: "Understand sequence gaps, caching, and snowflake-style IDs when insert throughput saturates nextval."
...
tags:
  ...
  - "Backend"
  - "Database"
keywords: "postgres sequence gap contention, production, backend"
faq:
  - q: "What problem does Postgres Sequence Gap and Contention solve?"
    a: "It addresses production gaps teams hit when scaling postgres sequence gap contention: correctness under concurrency, operability, and measurable SLOs instead of ad-hoc scripts."
  - q: "When should I adopt this pattern?"
    a: "Adopt when postgres sequence gap contention appears on incident timelines, p95 latency regresses, or the next traffic doubling will break the current shortcut."
...
A billing service lost duplicate events because postgres sequence gap contention was handled only in application code without database-enforced invariants. The fix was not more logging — it was moving the guarantee to the layer that survives process crashes and duplicate deliveries.

Senior backend work on postgres sequence gap and contention is less about syntax and more about failure modes: what happens on retry, on partial outage, and when two deploy versions run simultaneously during a rolling update.
...
Dashboards for postgres sequence gap contention should answer: 'Is the system slow, broken, or overloaded?' without SSH. Exemplars link spikes to trace IDs.
...
Measure before optimizing postgres sequence gap contention. Capture baseline p50/p95 latency, error rate, and resource utilization under representative load. Change one variable at a time — pool size, batch size, timeout, cache TTL — and re-measure.
...
Ship postgres sequence gap contention changes behind feature flags when behavior crosses service boundaries. Use canary deploys with automatic rollback on error rate or latency regression.
