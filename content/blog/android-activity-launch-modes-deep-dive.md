---
title: "Activity Launch Modes Deep Dive"
slug: "android-activity-launch-modes-deep-dive"
description: "standard, singleTop, singleTask, singleInstance: back stack behavior, deep link interactions, and task affinity."
datePublished: "2026-07-23"
dateModified: "2026-07-23"
tags: ["Android", "Activities", "Navigation"]
keywords: "activity launch modes Android, singleTask singleTop, back stack launch mode"
faq:
  - q: "When should Android teams adopt activity launch modes deep dive?"
    a: "Adopt activity launch modes deep dive when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with activity launch modes deep dive?"
  ...
  - q: "How do I debug activity launch modes deep dive issues in production?"
...
object AndroidActivityLaunchModesDeepDiveFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-activity-launch-modes-deep-dive_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for activity launch modes deep dive
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-activity-launch-modes-deep-dive` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get activity launch modes deep dive wrong in predictable ways:
...
When activity launch modes deep dive misbehaves in production:
...
Before enabling `android-activity-launch-modes-deep-dive` for all users:
...
## Activity Launch Modes Deep Dive on Samsung and Pixel divergence

Exercise activity launch modes deep dive on Galaxy A-series and Pixel a-series — emulators hide OEM battery and storage quirks. Capture Macrobenchmark or Firebase trace for the critical path touching activity; regressions above 8% block release for `android-activity-launch-modes-deep-dive`.

Document permission and background behavior in internal runbook: what breaks under Doze, what requires foreground service, and what Play policy declarations apply. Support tickets referencing "Activity Launch Modes Deep Dive" should map to a single runbook section with known workarounds.

## Modes regression gates for Play Vitals

Before promoting `android-activity-launch-modes-deep-dive` changes past 20% rollout, compare ANR rate, slow cold start, and excessive wakeups against seven-day baseline. Fail rollback review if dive path shows >5% increase in `slow frames` without documented trade-off approval.
