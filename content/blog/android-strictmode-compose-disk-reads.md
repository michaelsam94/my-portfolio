---
title: "StrictMode Disk Reads with Compose"
slug: "android-strictmode-compose-disk-reads"
description: "Catch accidental blocking I/O during composition: StrictMode policy, remember blocking calls anti-pattern."
datePublished: "2026-07-20"
dateModified: "2026-07-20"
tags: ["Android", "Performance", "StrictMode"]
keywords: "StrictMode Compose, disk read main thread Compose, StrictMode debug Android"
faq:
  - q: "When should Android teams adopt strictmode disk reads with compose?"
    a: "Adopt strictmode disk reads with compose when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with strictmode disk reads with compose?"
  ...
  - q: "How do I debug strictmode disk reads with compose issues in production?"
...
object AndroidStrictmodeComposeDiskReadsFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-strictmode-compose-disk-reads_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for strictmode disk reads with compose
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-strictmode-compose-disk-reads` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get strictmode disk reads with compose wrong in predictable ways:
...
When strictmode disk reads with compose misbehaves in production:
...
Before enabling `android-strictmode-compose-disk-reads` for all users:
