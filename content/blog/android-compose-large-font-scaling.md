---
title: "Large Font Scaling in Compose"
slug: "android-compose-large-font-scaling"
description: "Support 200% font scale: flexible layouts, max lines, vertical scroll, and avoiding text clipping in toolbars."
datePublished: "2026-08-15"
dateModified: "2026-08-15"
tags: ["Android", "Jetpack Compose", "Accessibility"]
keywords: "font scaling Compose, large text accessibility, 200 percent font scale Android"
faq:
  - q: "When should Android teams adopt large font scaling in compose?"
    a: "Adopt large font scaling in compose when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with large font scaling in compose?"
  ...
  - q: "How do I debug large font scaling in compose issues in production?"
...
object AndroidComposeLargeFontScalingFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-compose-large-font-scaling_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for large font scaling in compose
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-compose-large-font-scaling` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get large font scaling in compose wrong in predictable ways:
...
When large font scaling in compose misbehaves in production:
...
Before enabling `android-compose-large-font-scaling` for all users:
