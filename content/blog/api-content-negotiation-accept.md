---
title: "API Content Negotiation with Accept Headers"
slug: "api-content-negotiation-accept"
description: "Support JSON, Protobuf, or CSV via Accept — 406 handling and default representation policy."
...
tags:
  ...
  - "API"
  - "HTTP"
keywords: "api content negotiation accept, production, backend"
faq:
  - q: "What problem does API Content Negotiation solve?"
    a: "It addresses production gaps teams hit when scaling api content negotiation accept: correctness under concurrency, operability, and measurable SLOs instead of ad-hoc scripts."
  - q: "When should I adopt this pattern?"
    a: "Adopt when api content negotiation accept appears on incident timelines, p95 latency regresses, or the next traffic doubling will break the current shortcut."
...
A billing service lost duplicate events because api content negotiation accept was handled only in application code without database-enforced invariants. The fix was not more logging — it was moving the guarantee to the layer that survives process crashes and duplicate deliveries.

Senior backend work on api content negotiation with accept headers is less about syntax and more about failure modes: what happens on retry, on partial outage, and when two deploy versions run simultaneously during a rolling update.
...
Dashboards for api content negotiation accept should answer: 'Is the system slow, broken, or overloaded?' without SSH. Exemplars link spikes to trace IDs.
...
Measure before optimizing api content negotiation accept. Capture baseline p50/p95 latency, error rate, and resource utilization under representative load. Change one variable at a time — pool size, batch size, timeout, cache TTL — and re-measure.
...
Ship api content negotiation accept changes behind feature flags when behavior crosses service boundaries. Use canary deploys with automatic rollback on error rate or latency regression.
