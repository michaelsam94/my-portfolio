---
title: "Paging Placeholder Shimmer in Compose"
slug: "android-compose-paging-placeholder-shimmer"
description: "Shimmer placeholders for Paging LoadState.Loading: item count estimation and transition to real content."
datePublished: "2026-07-20"
dateModified: "2026-07-20"
tags: ["Android", "Jetpack Compose", "Paging"]
keywords: "paging shimmer Compose, LazyPagingItems placeholder, loading skeleton paging"
faq:
  - q: "When should Android teams adopt paging placeholder shimmer in compose?"
    a: "Adopt paging placeholder shimmer in compose when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with paging placeholder shimmer in compose?"
  ...
  - q: "How do I debug paging placeholder shimmer in compose issues in production?"
...
object AndroidComposePagingPlaceholderShimmerFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-compose-paging-placeholder-shimmer_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for paging placeholder shimmer in compose
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-compose-paging-placeholder-shimmer` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get paging placeholder shimmer in compose wrong in predictable ways:
...
When paging placeholder shimmer in compose misbehaves in production:
...
Before enabling `android-compose-paging-placeholder-shimmer` for all users:
