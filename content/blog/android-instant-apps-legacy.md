---
title: "Legacy Instant Apps and App Links Migration"
slug: "android-instant-apps-legacy"
description: "Instant Apps deprecation path: migrate to Play Instant or full install flows with deferred deep linking."
datePublished: "2026-07-24"
dateModified: "2026-07-24"
tags: ["Android", "Play Store", "Deep Links"]
keywords: "Instant Apps legacy, Google Play Instant, deferred deep link Android"
faq:
  - q: "When should Android teams adopt legacy instant apps and app links migration?"
    a: "Adopt legacy instant apps and app links migration when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with legacy instant apps and app links migration?"
  ...
  - q: "How do I debug legacy instant apps and app links migration issues in production?"
...
object AndroidInstantAppsLegacyFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-instant-apps-legacy_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for legacy instant apps and app links migration
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-instant-apps-legacy` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get legacy instant apps and app links migration wrong in predictable ways:
...
When legacy instant apps and app links migration misbehaves in production:
...
Before enabling `android-instant-apps-legacy` for all users:
