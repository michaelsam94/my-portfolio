---
title: "Custom Accessibility Actions in Compose"
slug: "android-compose-accessibility-custom-actions"
description: "semantics { customActions } for swipe actions, custom controls, and TalkBack discoverability beyond click."
datePublished: "2026-08-13"
dateModified: "2026-08-13"
tags: ["Android", "Jetpack Compose", "Accessibility"]
keywords: "custom accessibility actions Compose, semantics customActions, TalkBack Compose"
faq:
  - q: "When should Android teams adopt custom accessibility actions in compose?"
    a: "Adopt custom accessibility actions in compose when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with custom accessibility actions in compose?"
  ...
  - q: "How do I debug custom accessibility actions in compose issues in production?"
...
object AndroidComposeAccessibilityCustomActionsFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-compose-accessibility-custom-actions_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for custom accessibility actions in compose
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-compose-accessibility-custom-actions` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get custom accessibility actions in compose wrong in predictable ways:
...
When custom accessibility actions in compose misbehaves in production:
...
Before enabling `android-compose-accessibility-custom-actions` for all users:
