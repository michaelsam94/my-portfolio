---
title: "Implementing prefers-reduced-motion"
slug: "accessibility-reduced-motion-implementation"
description: "Respect reduced motion preference without stripping all animation — essential vs decorative motion patterns."
datePublished: "2026-08-05"
dateModified: "2026-08-05"
tags: ["Accessibility", "CSS", "Motion"]
keywords: "prefers-reduced-motion, accessible animation, motion sensitivity"
faq:
  - q: "What is Implementing prefers-reduced-motion?"
    a: "Implementing prefers-reduced-motion is a production pattern for frontend and product engineering teams building performant, accessible web applications. It addresses real constraints around user experience, security, and measurable outcomes — not theoretical best practices disconnected from shipping code."
  - q: "When should teams adopt Implementing prefers-reduced-motion?"
    a: "Adopt Implementing prefers-reduced-motion when you have field data or user research showing pain — slow interactions, accessibility gaps, conversion drop-offs, or security findings — and simpler fixes have been exhausted. Pilot on one route or feature before rolling out platform-wide."
  - q: "What are common mistakes with Implementing prefers-reduced-motion?"
...
Document which metrics you expect to move. If implementing prefers-reduced-motion is a performance change, baseline LCP, INP, and CLS in CrUX or your RUM tool for affected routes before merging. If it is an accessibility change, run axe and manual screen reader checks on the critical path — not just the component story.

## Implementation patterns

Start with the smallest change that proves the approach. For implementing prefers-reduced-motion, that usually means one route, one component tree, or one middleware rule — not a platform-wide migration.

```tsx
// Example: progressive adoption pattern
// Step 1 — isolate behind a feature flag or route segment
export async function Page() {
  const enabled = await flags.isEnabled("accessibility_reduced_motion_implementation");
...
Teams get implementing prefers-reduced-motion wrong in predictable ways:
...
When implementing prefers-reduced-motion misbehaves in production, work top-down:
