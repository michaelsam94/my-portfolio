---
title: "Kafka Tiered Storage for Archival"
slug: "kafka-tiered-storage-archival"
description: "Offload old segments to object storage — fetch latency and retention cost tradeoffs."
...
tags:
  ...
  - "Backend"
  - "Distributed Systems"
keywords: "kafka tiered storage archival, production, backend"
faq:
  - q: "What problem does Kafka Tiered Storage solve?"
    a: "It addresses production gaps teams hit when scaling kafka tiered storage archival: correctness under concurrency, operability, and measurable SLOs instead of ad-hoc scripts."
  - q: "When should I adopt this pattern?"
    a: "Adopt when kafka tiered storage archival appears on incident timelines, p95 latency regresses, or the next traffic doubling will break the current shortcut."
...
A billing service lost duplicate events because kafka tiered storage archival was handled only in application code without database-enforced invariants. The fix was not more logging — it was moving the guarantee to the layer that survives process crashes and duplicate deliveries.

Senior backend work on kafka tiered storage for archival is less about syntax and more about failure modes: what happens on retry, on partial outage, and when two deploy versions run simultaneously during a rolling update.
...
Dashboards for kafka tiered storage archival should answer: 'Is the system slow, broken, or overloaded?' without SSH. Exemplars link spikes to trace IDs.
...
Measure before optimizing kafka tiered storage archival. Capture baseline p50/p95 latency, error rate, and resource utilization under representative load. Change one variable at a time — pool size, batch size, timeout, cache TTL — and re-measure.
...
Ship kafka tiered storage archival changes behind feature flags when behavior crosses service boundaries. Use canary deploys with automatic rollback on error rate or latency regression.
