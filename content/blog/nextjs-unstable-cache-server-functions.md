---
title: "unstable_cache for Server Functions"
slug: "nextjs-unstable-cache-server-functions"
description: "Cache expensive server computations — key parts, revalidate tags, and dedupe across requests."
datePublished: "2026-12-31"
dateModified: "2026-12-31"
tags: ["Next.js", "Caching", "Server"]
keywords: "unstable_cache Next.js, server function cache, React cache"
faq:
  - q: "What is unstable_cache for Server Functions?"
    a: "unstable_cache for Server Functions is a production pattern for frontend and product engineering teams building performant, accessible web applications. It addresses real constraints around user experience, security, and measurable outcomes — not theoretical best practices disconnected from shipping code."
  - q: "When should teams adopt unstable_cache for Server Functions?"
    a: "Adopt unstable_cache for Server Functions when you have field data or user research showing pain — slow interactions, accessibility gaps, conversion drop-offs, or security findings — and simpler fixes have been exhausted. Pilot on one route or feature before rolling out platform-wide."
  - q: "What are common mistakes with unstable_cache for Server Functions?"
...
Document which metrics you expect to move. If unstable_cache for server functions is a performance change, baseline LCP, INP, and CLS in CrUX or your RUM tool for affected routes before merging. If it is an accessibility change, run axe and manual screen reader checks on the critical path — not just the component story.

## Implementation patterns

Start with the smallest change that proves the approach. For unstable_cache for server functions, that usually means one route, one component tree, or one middleware rule — not a platform-wide migration.

```tsx
// Example: progressive adoption pattern
// Step 1 — isolate behind a feature flag or route segment
export async function Page() {
  const enabled = await flags.isEnabled("nextjs_unstable_cache_server_functions");
...
Teams get unstable_cache for server functions wrong in predictable ways:
...
When unstable_cache for server functions misbehaves in production, work top-down:
