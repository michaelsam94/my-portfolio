---
title: "BiometricPrompt with CryptoObject"
slug: "android-biometric-crypto-object"
description: "Bind BiometricPrompt to Keystore Cipher: Class 3 biometrics, invalidation on enrollment change, and fallback PIN."
datePublished: "2026-08-06"
dateModified: "2026-08-06"
tags: ["Android", "Security", "Biometrics"]
keywords: "BiometricPrompt CryptoObject, biometric encryption Android, Class 3 biometric"
faq:
  - q: "When should Android teams adopt biometricprompt with cryptoobject?"
    a: "Adopt biometricprompt with cryptoobject when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with biometricprompt with cryptoobject?"
  ...
  - q: "How do I debug biometricprompt with cryptoobject issues in production?"
...
object AndroidBiometricCryptoObjectFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-biometric-crypto-object_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for biometricprompt with cryptoobject
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-biometric-crypto-object` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get biometricprompt with cryptoobject wrong in predictable ways:
...
When biometricprompt with cryptoobject misbehaves in production:
...
Before enabling `android-biometric-crypto-object` for all users:
...
## Biometric Crypto Object on Samsung and Pixel divergence

Exercise biometric crypto object on Galaxy A-series and Pixel a-series — emulators hide OEM battery and storage quirks. Capture Macrobenchmark or Firebase trace for the critical path touching biometric; regressions above 8% block release for `android-biometric-crypto-object`.

Document permission and background behavior in internal runbook: what breaks under Doze, what requires foreground service, and what Play policy declarations apply. Support tickets referencing "Biometric Crypto Object" should map to a single runbook section with known workarounds.

## Crypto regression gates for Play Vitals

Before promoting `android-biometric-crypto-object` changes past 20% rollout, compare ANR rate, slow cold start, and excessive wakeups against seven-day baseline. Fail rollback review if object path shows >5% increase in `slow frames` without documented trade-off approval.
