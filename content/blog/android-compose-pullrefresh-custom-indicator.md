---
title: "Custom Pull Refresh Indicator"
slug: "android-compose-pullrefresh-custom-indicator"
description: "PullToRefreshBox customization: indicator offset, nested scroll, and Material 3 pull refresh migration."
datePublished: "2026-07-22"
dateModified: "2026-07-22"
tags: ["Android", "Jetpack Compose", "Material 3"]
keywords: "pull refresh Compose custom, PullToRefreshBox indicator, pull to refresh Material 3"
faq:
  - q: "When should Android teams adopt custom pull refresh indicator?"
    a: "Adopt custom pull refresh indicator when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with custom pull refresh indicator?"
  ...
  - q: "How do I debug custom pull refresh indicator issues in production?"
...
object AndroidComposePullrefreshCustomIndicatorFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-compose-pullrefresh-custom-indicator_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for custom pull refresh indicator
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-compose-pullrefresh-custom-indicator` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get custom pull refresh indicator wrong in predictable ways:
...
When custom pull refresh indicator misbehaves in production:
...
Before enabling `android-compose-pullrefresh-custom-indicator` for all users:
