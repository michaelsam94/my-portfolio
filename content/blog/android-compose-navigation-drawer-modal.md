---
title: "Modal Navigation Drawer in Compose"
slug: "android-compose-navigation-drawer-modal"
description: "ModalNavigationDrawer gestures: edge swipe width, predictive back, and permanent vs modal drawer selection."
datePublished: "2026-08-11"
dateModified: "2026-08-11"
tags: ["Android", "Jetpack Compose", "Navigation"]
keywords: "ModalNavigationDrawer Compose, navigation drawer gesture, drawer predictive back"
faq:
  - q: "When should Android teams adopt modal navigation drawer in compose?"
    a: "Adopt modal navigation drawer in compose when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with modal navigation drawer in compose?"
  ...
  - q: "How do I debug modal navigation drawer in compose issues in production?"
...
object AndroidComposeNavigationDrawerModalFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-compose-navigation-drawer-modal_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for modal navigation drawer in compose
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-compose-navigation-drawer-modal` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get modal navigation drawer in compose wrong in predictable ways:
...
When modal navigation drawer in compose misbehaves in production:
...
Before enabling `android-compose-navigation-drawer-modal` for all users:
