---
title: "Heads-Up Notification Control"
slug: "android-heads-up-notification-control"
description: "When heads-up shows: fullScreenIntent, high priority FCM, category CALL, and avoiding notification fatigue."
datePublished: "2026-08-12"
dateModified: "2026-08-12"
tags: ["Android", "Notifications", "UX"]
keywords: "heads-up notification Android, high priority notification, notification heads up control"
faq:
  - q: "When should Android teams adopt heads-up notification control?"
    a: "Adopt heads-up notification control when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with heads-up notification control?"
  ...
  - q: "How do I debug heads-up notification control issues in production?"
...
object AndroidHeadsUpNotificationControlFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-heads-up-notification-control_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for heads-up notification control
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-heads-up-notification-control` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get heads-up notification control wrong in predictable ways:
...
When heads-up notification control misbehaves in production:
...
Before enabling `android-heads-up-notification-control` for all users:
