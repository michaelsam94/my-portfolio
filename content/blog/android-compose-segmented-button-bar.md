---
title: "Segmented Button Bar in Compose"
slug: "android-compose-segmented-button-bar"
description: "Single-select SegmentedButtonRow: icon segments, accessibility selected state, and replacing legacy TabRow toggles."
datePublished: "2026-08-05"
dateModified: "2026-08-05"
tags: ["Android", "Jetpack Compose", "Material 3"]
keywords: "SegmentedButton Compose, segmented control Material 3, toggle segment bar"
faq:
  - q: "When should Android teams adopt segmented button bar in compose?"
  ...
  - q: "What are the most common mistakes with segmented button bar in compose?"
  ...
  - q: "How do I debug segmented button bar in compose issues in production?"
...
object AndroidComposeSegmentedButtonBarFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-compose-segmented-button-bar_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for segmented button bar in compose
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-compose-segmented-button-bar` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get segmented button bar in compose wrong in predictable ways:
...
When segmented button bar in compose misbehaves in production:
...
Before enabling `android-compose-segmented-button-bar` for all users:
