---
title: "Play App Signing and Upload Keys"
slug: "android-app-signing-play-app-signing"
description: "Manage upload keys vs app signing keys: key rotation, PEPK export, and recovery when upload key is lost."
datePublished: "2026-07-29"
dateModified: "2026-07-29"
tags: ["Android", "Play Store", "Security"]
keywords: "Play App Signing, upload key rotation, app signing key Android"
faq:
  - q: "When should Android teams adopt play app signing and upload keys?"
    a: "Adopt play app signing and upload keys when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with play app signing and upload keys?"
  ...
  - q: "How do I debug play app signing and upload keys issues in production?"
...
object AndroidAppSigningPlayAppSigningFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-app-signing-play-app-signing_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for play app signing and upload keys
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-app-signing-play-app-signing` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get play app signing and upload keys wrong in predictable ways:
...
When play app signing and upload keys misbehaves in production:
...
Before enabling `android-app-signing-play-app-signing` for all users:
...
## App Signing Play App Signing on Samsung and Pixel divergence

Exercise app signing play app signing on Galaxy A-series and Pixel a-series — emulators hide OEM battery and storage quirks. Capture Macrobenchmark or Firebase trace for the critical path touching app; regressions above 8% block release for `android-app-signing-play-app-signing`.

Document permission and background behavior in internal runbook: what breaks under Doze, what requires foreground service, and what Play policy declarations apply. Support tickets referencing "App Signing Play App Signing" should map to a single runbook section with known workarounds.

## Play regression gates for Play Vitals

Before promoting `android-app-signing-play-app-signing` changes past 20% rollout, compare ANR rate, slow cold start, and excessive wakeups against seven-day baseline. Fail rollback review if signing path shows >5% increase in `slow frames` without documented trade-off approval.
