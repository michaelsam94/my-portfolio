---
title: "Nearby Share API Integration"
slug: "android-nearby-share-api"
description: "Share files with Nearby Share intents: Fast Share compatibility, large payload handling, and privacy UX."
datePublished: "2026-08-06"
dateModified: "2026-08-06"
tags: ["Android", "Nearby", "UX"]
keywords: "Nearby Share Android, share sheet Android, direct share nearby"
faq:
  - q: "When should Android teams adopt nearby share api integration?"
    a: "Adopt nearby share api integration when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with nearby share api integration?"
  ...
  - q: "How do I debug nearby share api integration issues in production?"
...
object AndroidNearbyShareApiFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-nearby-share-api_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for nearby share api integration
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-nearby-share-api` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get nearby share api integration wrong in predictable ways:
...
When nearby share api integration misbehaves in production:
...
Before enabling `android-nearby-share-api` for all users:
