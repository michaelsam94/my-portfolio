---
title: "Captive Portal Detection Handling"
slug: "android-captive-portal-detection"
description: "Respond to NET_CAPABILITY_CAPTIVE_PORTAL: custom login flow, avoid sync storms on hotel Wi-Fi."
datePublished: "2026-08-11"
dateModified: "2026-08-11"
tags: ["Android", "Networking", "Wi-Fi"]
keywords: "captive portal Android, captive portal detection, hotel Wi-Fi app Android"
faq:
  - q: "When should Android teams adopt captive portal detection handling?"
    a: "Adopt captive portal detection handling when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with captive portal detection handling?"
  ...
  - q: "How do I debug captive portal detection handling issues in production?"
...
object AndroidCaptivePortalDetectionFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-captive-portal-detection_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for captive portal detection handling
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-captive-portal-detection` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get captive portal detection handling wrong in predictable ways:
...
When captive portal detection handling misbehaves in production:
...
Before enabling `android-captive-portal-detection` for all users:
...
## Captive Portal Detection on Samsung and Pixel divergence

Exercise captive portal detection on Galaxy A-series and Pixel a-series — emulators hide OEM battery and storage quirks. Capture Macrobenchmark or Firebase trace for the critical path touching captive; regressions above 8% block release for `android-captive-portal-detection`.

Document permission and background behavior in internal runbook: what breaks under Doze, what requires foreground service, and what Play policy declarations apply. Support tickets referencing "Captive Portal Detection" should map to a single runbook section with known workarounds.

## Portal regression gates for Play Vitals

Before promoting `android-captive-portal-detection` changes past 20% rollout, compare ANR rate, slow cold start, and excessive wakeups against seven-day baseline. Fail rollback review if detection path shows >5% increase in `slow frames` without documented trade-off approval.
