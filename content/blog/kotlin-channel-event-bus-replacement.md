---
title: "Replace Event Bus with Channel Flow"
slug: "kotlin-channel-event-bus-replacement"
description: "BroadcastChannel is deprecated — Channel, SharedFlow events, and avoiding memory leaks from sticky events."
datePublished: "2026-07-25"
dateModified: "2026-07-25"
tags: ["Kotlin", "Coroutines", "Architecture"]
keywords: "Channel event bus Kotlin, SharedFlow events Android, BroadcastChannel replacement"
faq:
  - q: "When should Android teams adopt replace event bus with channel flow?"
    a: "Adopt replace event bus with channel flow when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with replace event bus with channel flow?"
  ...
  - q: "How do I debug replace event bus with channel flow issues in production?"
...
object KotlinChannelEventBusReplacementFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("kotlin-channel-event-bus-replacement_enabled", default = false)
}
...
class KotlinRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for replace event bus with channel flow
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `kotlin-channel-event-bus-replacement` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get replace event bus with channel flow wrong in predictable ways:
...
When replace event bus with channel flow misbehaves in production:
...
Before enabling `kotlin-channel-event-bus-replacement` for all users:
