---
title: "FCM Data Messages vs Notification Messages"
slug: "android-push-fcm-data-messages"
description: "Handle FCM data payloads in background: onMessageReceived, notification delegation, and high-priority trade-offs."
datePublished: "2026-08-01"
dateModified: "2026-08-01"
tags: ["Android", "FCM", "Push"]
keywords: "FCM data messages, notification messages Android, Firebase Cloud Messaging background"
faq:
  - q: "When should Android teams adopt fcm data messages vs notification messages?"
    a: "Adopt fcm data messages vs notification messages when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with fcm data messages vs notification messages?"
  ...
  - q: "How do I debug fcm data messages vs notification messages issues in production?"
...
object AndroidPushFcmDataMessagesFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-push-fcm-data-messages_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for fcm data messages vs notification messages
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-push-fcm-data-messages` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get fcm data messages vs notification messages wrong in predictable ways:
...
When fcm data messages vs notification messages misbehaves in production:
...
Before enabling `android-push-fcm-data-messages` for all users:
