...
faq:
  ...
  - q: "Can agent conversation memory be separate from marketing consent?"
    a: "Yes — and it must be. Functional memory (remember my preferences for this product) and secondary processing (model training, marketing personalization) require separate purposes with separate toggles. Bundling them invalidates consent under GDPR and CPRA opt-out rules. Your consent record schema should model purpose as a first-class dimension, not a boolean."
---
...
## Purpose taxonomy for agent workloads
...
Map every agent tool to required purposes:
...
CI validates that every registered tool declares purposes. Undeclared tools fail deploy — the agent runtime refuses to register them.
