---
title: "Exact Alarm Permission User Flow"
slug: "android-exact-alarm-permission-flow"
description: "SCHEDULE_EXACT_ALARM and USE_EXACT_ALARM: Settings intent, graceful degradation, and Play policy compliance."
datePublished: "2026-08-09"
dateModified: "2026-08-09"
tags: ["Android", "AlarmManager", "Permissions"]
keywords: "exact alarm permission Android, SCHEDULE_EXACT_ALARM UX, alarm permission flow"
faq:
  - q: "When should Android teams adopt exact alarm permission user flow?"
    a: "Adopt exact alarm permission user flow when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with exact alarm permission user flow?"
  ...
  - q: "How do I debug exact alarm permission user flow issues in production?"
...
object AndroidExactAlarmPermissionFlowFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-exact-alarm-permission-flow_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for exact alarm permission user flow
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-exact-alarm-permission-flow` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get exact alarm permission user flow wrong in predictable ways:
...
When exact alarm permission user flow misbehaves in production:
...
Before enabling `android-exact-alarm-permission-flow` for all users:
...
## Exact Alarm Permission Flow on Samsung and Pixel divergence

Exercise exact alarm permission flow on Galaxy A-series and Pixel a-series — emulators hide OEM battery and storage quirks. Capture Macrobenchmark or Firebase trace for the critical path touching exact; regressions above 8% block release for `android-exact-alarm-permission-flow`.

Document permission and background behavior in internal runbook: what breaks under Doze, what requires foreground service, and what Play policy declarations apply. Support tickets referencing "Exact Alarm Permission Flow" should map to a single runbook section with known workarounds.

## Permission regression gates for Play Vitals

Before promoting `android-exact-alarm-permission-flow` changes past 20% rollout, compare ANR rate, slow cold start, and excessive wakeups against seven-day baseline. Fail rollback review if flow path shows >5% increase in `slow frames` without documented trade-off approval.
