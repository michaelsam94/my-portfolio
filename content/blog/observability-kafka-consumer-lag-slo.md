---
title: "Kafka Consumer Lag SLO"
slug: "observability-kafka-consumer-lag-slo"
description: "Define lag SLO per consumer group — burn rate alerts and capacity planning."
...
tags:
  ...
  - "Backend"
  - "SRE"
keywords: "observability kafka consumer lag slo, production, backend"
faq:
  - q: "What problem does Kafka Consumer Lag SLO solve?"
    a: "It addresses production gaps teams hit when scaling observability kafka consumer lag slo: correctness under concurrency, operability, and measurable SLOs instead of ad-hoc scripts."
  - q: "When should I adopt this pattern?"
    a: "Adopt when observability kafka consumer lag slo appears on incident timelines, p95 latency regresses, or the next traffic doubling will break the current shortcut."
...
A billing service lost duplicate events because observability kafka consumer lag slo was handled only in application code without database-enforced invariants. The fix was not more logging — it was moving the guarantee to the layer that survives process crashes and duplicate deliveries.

Senior backend work on kafka consumer lag slo is less about syntax and more about failure modes: what happens on retry, on partial outage, and when two deploy versions run simultaneously during a rolling update.
...
Dashboards for observability kafka consumer lag slo should answer: 'Is the system slow, broken, or overloaded?' without SSH. Exemplars link spikes to trace IDs.
...
Measure before optimizing observability kafka consumer lag slo. Capture baseline p50/p95 latency, error rate, and resource utilization under representative load. Change one variable at a time — pool size, batch size, timeout, cache TTL — and re-measure.
...
Ship observability kafka consumer lag slo changes behind feature flags when behavior crosses service boundaries. Use canary deploys with automatic rollback on error rate or latency regression.
