---
title: "Database Migration Backfill Batching"
slug: "database-migration-backfill-batching"
description: "Batch UPDATE with sleep — avoid long locks and replication lag spikes on large tables."
...
tags:
  ...
  - "Backend"
  - "Migrations"
keywords: "database migration backfill batching, production, backend"
faq:
  - q: "What problem does Database Migration Backfill Batching solve?"
    a: "It addresses production gaps teams hit when scaling database migration backfill batching: correctness under concurrency, operability, and measurable SLOs instead of ad-hoc scripts."
  - q: "When should I adopt this pattern?"
    a: "Adopt when database migration backfill batching appears on incident timelines, p95 latency regresses, or the next traffic doubling will break the current shortcut."
...
A billing service lost duplicate events because database migration backfill batching was handled only in application code without database-enforced invariants. The fix was not more logging — it was moving the guarantee to the layer that survives process crashes and duplicate deliveries.

Senior backend work on database migration backfill batching is less about syntax and more about failure modes: what happens on retry, on partial outage, and when two deploy versions run simultaneously during a rolling update.
...
Dashboards for database migration backfill batching should answer: 'Is the system slow, broken, or overloaded?' without SSH. Exemplars link spikes to trace IDs.
...
Measure before optimizing database migration backfill batching. Capture baseline p50/p95 latency, error rate, and resource utilization under representative load. Change one variable at a time — pool size, batch size, timeout, cache TTL — and re-measure.
...
Ship database migration backfill batching changes behind feature flags when behavior crosses service boundaries. Use canary deploys with automatic rollback on error rate or latency regression.
