---
title: "Swipeable List Actions in Compose"
slug: "android-compose-swipeable-list-actions"
description: "SwipeToDismissBox, anchoredDraggable for mail/archive actions, and accessibility custom swipe actions."
datePublished: "2026-07-21"
dateModified: "2026-07-21"
tags: ["Android", "Jetpack Compose", "UX"]
keywords: "swipe to dismiss Compose, list swipe actions Android, SwipeToDismissBox"
faq:
  - q: "When should Android teams adopt swipeable list actions in compose?"
    a: "Adopt swipeable list actions in compose when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with swipeable list actions in compose?"
  ...
  - q: "How do I debug swipeable list actions in compose issues in production?"
...
object AndroidComposeSwipeableListActionsFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-compose-swipeable-list-actions_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for swipeable list actions in compose
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-compose-swipeable-list-actions` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get swipeable list actions in compose wrong in predictable ways:
...
When swipeable list actions in compose misbehaves in production:
...
Before enabling `android-compose-swipeable-list-actions` for all users:
