---
title: "Short Foreground Service Exemption"
slug: "android-short-service-exemption"
description: "Android 15 short FGS: six-minute window, eligible use cases, and migrating long tasks to WorkManager."
datePublished: "2026-07-18"
dateModified: "2026-07-18"
tags: ["Android", "Foreground Service", "Android 15"]
keywords: "short foreground service Android, FGS timeout Android 15, six minute FGS"
faq:
  - q: "When should Android teams adopt short foreground service exemption?"
    a: "Adopt short foreground service exemption when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with short foreground service exemption?"
  ...
  - q: "How do I debug short foreground service exemption issues in production?"
...
object AndroidShortServiceExemptionFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-short-service-exemption_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for short foreground service exemption
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-short-service-exemption` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get short foreground service exemption wrong in predictable ways:
...
When short foreground service exemption misbehaves in production:
...
Before enabling `android-short-service-exemption` for all users:
