---
title: "Distributed Rate Limiting with Redis Lua"
slug: "rate-limit-distributed-redis-lua"
description: "Atomic token bucket in Lua — sliding window log vs fixed window accuracy."
...
tags:
  ...
  - "API"
  - "Rate Limiting"
keywords: "rate limit distributed redis lua, production, backend"
faq:
  - q: "What problem does Distributed Rate Limiting solve?"
    a: "It addresses production gaps teams hit when scaling rate limit distributed redis lua: correctness under concurrency, operability, and measurable SLOs instead of ad-hoc scripts."
  - q: "When should I adopt this pattern?"
    a: "Adopt when rate limit distributed redis lua appears on incident timelines, p95 latency regresses, or the next traffic doubling will break the current shortcut."
...
A billing service lost duplicate events because rate limit distributed redis lua was handled only in application code without database-enforced invariants. The fix was not more logging — it was moving the guarantee to the layer that survives process crashes and duplicate deliveries.

Senior backend work on distributed rate limiting with redis lua is less about syntax and more about failure modes: what happens on retry, on partial outage, and when two deploy versions run simultaneously during a rolling update.
...
Dashboards for rate limit distributed redis lua should answer: 'Is the system slow, broken, or overloaded?' without SSH. Exemplars link spikes to trace IDs.
...
Measure before optimizing rate limit distributed redis lua. Capture baseline p50/p95 latency, error rate, and resource utilization under representative load. Change one variable at a time — pool size, batch size, timeout, cache TTL — and re-measure.
...
Ship rate limit distributed redis lua changes behind feature flags when behavior crosses service boundaries. Use canary deploys with automatic rollback on error rate or latency regression.
