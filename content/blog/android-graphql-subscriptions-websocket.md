...
slug: "android-graphql-subscriptions-websocket"
description: "Apollo subscription transport: WebSocket reconnect, auth headers refresh, and battery-aware subscription lifecycle."
datePublished: "2026-07-27"
dateModified: "2026-07-27"
tags: ["Android", "GraphQL", "WebSocket"]
keywords: "GraphQL subscriptions Android, Apollo WebSocket, subscription reconnect mobile"
faq:
  - q: "When should Android teams adopt graphql subscriptions over websocket?"
    a: "Adopt graphql subscriptions over websocket when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with graphql subscriptions over websocket?"
  ...
  - q: "How do I debug graphql subscriptions over websocket issues in production?"
...
object AndroidGraphqlSubscriptionsWebsocketFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-graphql-subscriptions-websocket_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for graphql subscriptions over websocket
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-graphql-subscriptions-websocket` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get graphql subscriptions over websocket wrong in predictable ways:
...
When graphql subscriptions over websocket misbehaves in production:
...
Before enabling `android-graphql-subscriptions-websocket` for all users:
