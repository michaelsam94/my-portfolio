---
title: "Why Service Locator Is an Anti-Pattern on Android"
slug: "android-servicelocator-antipattern-di"
description: "Service locator vs constructor injection: testability, compile-time safety, and migrating legacy singletons to Hilt."
datePublished: "2026-07-31"
dateModified: "2026-07-31"
tags: ["Android", "Architecture", "Hilt"]
keywords: "Service locator anti-pattern, dependency injection Android, Hilt migration"
faq:
  - q: "When should Android teams adopt why service locator is an anti-pattern on android?"
    a: "Adopt why service locator is an anti-pattern on android when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with why service locator is an anti-pattern on android?"
  ...
  - q: "How do I debug why service locator is an anti-pattern on android issues in production?"
...
object AndroidServicelocatorAntipatternDiFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-servicelocator-antipattern-di_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for why service locator is an anti-pattern on android
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-servicelocator-antipattern-di` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get why service locator is an anti-pattern on android wrong in predictable ways:
...
When why service locator is an anti-pattern on android misbehaves in production:
...
Before enabling `android-servicelocator-antipattern-di` for all users:
