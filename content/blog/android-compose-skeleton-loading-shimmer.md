---
title: "Skeleton Loading Shimmer in Compose"
slug: "android-compose-skeleton-loading-shimmer"
description: "Shimmer effect with Modifier.graphicsLayer and infinite animation — respect reduced motion with static skeleton."
datePublished: "2026-08-01"
dateModified: "2026-08-01"
tags: ["Android", "Jetpack Compose", "UX"]
keywords: "shimmer skeleton Compose, loading placeholder shimmer, skeleton loading Android"
faq:
  - q: "When should Android teams adopt skeleton loading shimmer in compose?"
    a: "Adopt skeleton loading shimmer in compose when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with skeleton loading shimmer in compose?"
  ...
  - q: "How do I debug skeleton loading shimmer in compose issues in production?"
...
object AndroidComposeSkeletonLoadingShimmerFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-compose-skeleton-loading-shimmer_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for skeleton loading shimmer in compose
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-compose-skeleton-loading-shimmer` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get skeleton loading shimmer in compose wrong in predictable ways:
...
When skeleton loading shimmer in compose misbehaves in production:
...
Before enabling `android-compose-skeleton-loading-shimmer` for all users:
