...
slug: "android-do-not-disturb-policy-access"
description: "NotificationManager policy access: priority categories, automatic rule respect, and alarm bypass for critical alerts."
datePublished: "2026-08-10"
dateModified: "2026-08-10"
tags: ["Android", "Notifications", "System"]
keywords: "Do Not Disturb Android, notification policy access, DND priority Android"
faq:
  - q: "When should Android teams adopt do not disturb policy access?"
    a: "Adopt do not disturb policy access when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with do not disturb policy access?"
  ...
  - q: "How do I debug do not disturb policy access issues in production?"
...
object AndroidDoNotDisturbPolicyAccessFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-do-not-disturb-policy-access_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for do not disturb policy access
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-do-not-disturb-policy-access` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get do not disturb policy access wrong in predictable ways:
...
When do not disturb policy access misbehaves in production:
...
Before enabling `android-do-not-disturb-policy-access` for all users:
...
## Do Not Disturb Policy Access on Samsung and Pixel divergence

Exercise do not disturb policy access on Galaxy A-series and Pixel a-series — emulators hide OEM battery and storage quirks. Capture Macrobenchmark or Firebase trace for the critical path touching do; regressions above 8% block release for `android-do-not-disturb-policy-access`.

Document permission and background behavior in internal runbook: what breaks under Doze, what requires foreground service, and what Play policy declarations apply. Support tickets referencing "Do Not Disturb Policy Access" should map to a single runbook section with known workarounds.

## Disturb regression gates for Play Vitals

Before promoting `android-do-not-disturb-policy-access` changes past 20% rollout, compare ANR rate, slow cold start, and excessive wakeups against seven-day baseline. Fail rollback review if access path shows >5% increase in `slow frames` without documented trade-off approval.
