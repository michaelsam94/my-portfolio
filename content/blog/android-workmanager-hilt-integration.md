...
description: "HiltWorkerFactory, @AssistedInject workers, and injecting repositories into background jobs safely."
datePublished: "2026-08-09"
dateModified: "2026-08-09"
tags: ["Android", "WorkManager", "Hilt"]
keywords: "WorkManager Hilt, HiltWorkerFactory, assisted inject worker Android"
faq:
  - q: "When should Android teams adopt workmanager hilt integration?"
    a: "Adopt workmanager hilt integration when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with workmanager hilt integration?"
  ...
  - q: "How do I debug workmanager hilt integration issues in production?"
...
object AndroidWorkmanagerHiltIntegrationFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-workmanager-hilt-integration_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for workmanager hilt integration
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-workmanager-hilt-integration` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get workmanager hilt integration wrong in predictable ways:
...
When workmanager hilt integration misbehaves in production:
...
Before enabling `android-workmanager-hilt-integration` for all users:
