---
title: "ExoPlayer Surface in Compose"
slug: "android-compose-videoview-exoplayer"
description: "AndroidView PlayerView with ExoPlayer: lifecycle pause, PiP handoff, and Compose state for playback controls."
datePublished: "2026-07-21"
dateModified: "2026-07-21"
tags: ["Android", "Jetpack Compose", "Media"]
keywords: "ExoPlayer Compose, PlayerView AndroidView, video playback Compose"
faq:
  - q: "When should Android teams adopt exoplayer surface in compose?"
    a: "Adopt exoplayer surface in compose when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with exoplayer surface in compose?"
  ...
  - q: "How do I debug exoplayer surface in compose issues in production?"
...
object AndroidComposeVideoviewExoplayerFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-compose-videoview-exoplayer_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for exoplayer surface in compose
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-compose-videoview-exoplayer` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get exoplayer surface in compose wrong in predictable ways:
...
When exoplayer surface in compose misbehaves in production:
...
Before enabling `android-compose-videoview-exoplayer` for all users:
