---
title: "loading.tsx and Error Boundary Architecture"
slug: "nextjs-loading-ui-error-boundaries"
description: "Route-level loading and error UI — error.tsx boundaries, not-found.tsx, and nested segment error isolation."
datePublished: "2026-12-28"
dateModified: "2026-12-28"
tags: ["Next.js", "Error Handling", "UX"]
keywords: "Next.js loading.tsx, error.tsx App Router, error boundary Next.js"
faq:
  - q: "What is loading.tsx and Error Boundary Architecture?"
    a: "loading.tsx and Error Boundary Architecture is a production pattern for frontend and product engineering teams building performant, accessible web applications. It addresses real constraints around user experience, security, and measurable outcomes — not theoretical best practices disconnected from shipping code."
  - q: "When should teams adopt loading.tsx and Error Boundary Architecture?"
    a: "Adopt loading.tsx and Error Boundary Architecture when you have field data or user research showing pain — slow interactions, accessibility gaps, conversion drop-offs, or security findings — and simpler fixes have been exhausted. Pilot on one route or feature before rolling out platform-wide."
  - q: "What are common mistakes with loading.tsx and Error Boundary Architecture?"
...
Document which metrics you expect to move. If loading.tsx and error boundary architecture is a performance change, baseline LCP, INP, and CLS in CrUX or your RUM tool for affected routes before merging. If it is an accessibility change, run axe and manual screen reader checks on the critical path — not just the component story.

## Implementation patterns

Start with the smallest change that proves the approach. For loading.tsx and error boundary architecture, that usually means one route, one component tree, or one middleware rule — not a platform-wide migration.

```tsx
// Example: progressive adoption pattern
// Step 1 — isolate behind a feature flag or route segment
export async function Page() {
  const enabled = await flags.isEnabled("nextjs_loading_ui_error_boundaries");
...
Teams get loading.tsx and error boundary architecture wrong in predictable ways:
...
When loading.tsx and error boundary architecture misbehaves in production, work top-down:
