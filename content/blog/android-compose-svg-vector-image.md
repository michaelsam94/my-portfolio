---
title: "SVG and Vector Images in Compose"
slug: "android-compose-svg-vector-image"
description: "VectorDrawable in Compose, Coil SVG decoder, and tinting icons with MaterialTheme color roles."
datePublished: "2026-08-04"
dateModified: "2026-08-04"
tags: ["Android", "Jetpack Compose", "Graphics"]
keywords: "SVG Compose Android, vector image Compose, VectorDrawable Compose"
faq:
  - q: "When should Android teams adopt svg and vector images in compose?"
    a: "Adopt svg and vector images in compose when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with svg and vector images in compose?"
  ...
  - q: "How do I debug svg and vector images in compose issues in production?"
...
object AndroidComposeSvgVectorImageFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-compose-svg-vector-image_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for svg and vector images in compose
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-compose-svg-vector-image` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get svg and vector images in compose wrong in predictable ways:
...
When svg and vector images in compose misbehaves in production:
...
Before enabling `android-compose-svg-vector-image` for all users:
