---
title: "Database Deferred Foreign Key Constraints"
slug: "database-migration-foreign-key-deferred"
description: "DEFERRABLE INITIALLY DEFERRED for bulk loads — tradeoffs vs immediate integrity."
...
tags:
  ...
  - "Backend"
  - "Migrations"
keywords: "database migration foreign key deferred, production, backend"
faq:
  - q: "What problem does Database Deferred Foreign Key Constraints solve?"
    a: "It addresses production gaps teams hit when scaling database migration foreign key deferred: correctness under concurrency, operability, and measurable SLOs instead of ad-hoc scripts."
  - q: "When should I adopt this pattern?"
    a: "Adopt when database migration foreign key deferred appears on incident timelines, p95 latency regresses, or the next traffic doubling will break the current shortcut."
...
A billing service lost duplicate events because database migration foreign key deferred was handled only in application code without database-enforced invariants. The fix was not more logging — it was moving the guarantee to the layer that survives process crashes and duplicate deliveries.

Senior backend work on database deferred foreign key constraints is less about syntax and more about failure modes: what happens on retry, on partial outage, and when two deploy versions run simultaneously during a rolling update.
...
Dashboards for database migration foreign key deferred should answer: 'Is the system slow, broken, or overloaded?' without SSH. Exemplars link spikes to trace IDs.
...
Measure before optimizing database migration foreign key deferred. Capture baseline p50/p95 latency, error rate, and resource utilization under representative load. Change one variable at a time — pool size, batch size, timeout, cache TTL — and re-measure.
...
Ship database migration foreign key deferred changes behind feature flags when behavior crosses service boundaries. Use canary deploys with automatic rollback on error rate or latency regression.
