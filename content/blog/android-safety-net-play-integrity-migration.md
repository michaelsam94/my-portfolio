---
title: "Migrating SafetyNet to Play Integrity API"
slug: "android-safety-net-play-integrity-migration"
description: "Replace SafetyNet Attestation with Play Integrity: standard vs classic requests, server-side verdict parsing."
datePublished: "2026-07-31"
dateModified: "2026-07-31"
tags: ["Android", "Security", "Play Integrity"]
keywords: "Play Integrity API, SafetyNet migration, device attestation Android"
faq:
  - q: "When should Android teams adopt migrating safetynet to play integrity api?"
    a: "Adopt migrating safetynet to play integrity api when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with migrating safetynet to play integrity api?"
  ...
  - q: "How do I debug migrating safetynet to play integrity api issues in production?"
...
object AndroidSafetyNetPlayIntegrityMigrationFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-safety-net-play-integrity-migration_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for migrating safetynet to play integrity api
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-safety-net-play-integrity-migration` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get migrating safetynet to play integrity api wrong in predictable ways:
...
When migrating safetynet to play integrity api misbehaves in production:
...
Before enabling `android-safety-net-play-integrity-migration` for all users:
