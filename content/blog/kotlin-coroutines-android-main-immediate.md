---
title: "Main.immediate on Android"
slug: "kotlin-coroutines-android-main-immediate"
description: "Dispatchers.Main.immediate vs Main: reentrant UI updates, test dispatchers, and avoiding double frame delay."
datePublished: "2026-07-21"
dateModified: "2026-07-21"
tags: ["Kotlin", "Coroutines", "Android"]
keywords: "Main.immediate Android, Dispatchers Main immediate, reentrant coroutine UI"
faq:
  - q: "When should Android teams adopt main.immediate on android?"
    a: "Adopt main.immediate on android when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with main.immediate on android?"
  ...
  - q: "How do I debug main.immediate on android issues in production?"
...
object KotlinCoroutinesAndroidMainImmediateFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("kotlin-coroutines-android-main-immediate_enabled", default = false)
}
...
class KotlinRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for main.immediate on android
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `kotlin-coroutines-android-main-immediate` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get main.immediate on android wrong in predictable ways:
...
When main.immediate on android misbehaves in production:
...
Before enabling `kotlin-coroutines-android-main-immediate` for all users:
