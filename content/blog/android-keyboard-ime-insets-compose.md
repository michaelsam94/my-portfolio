---
title: "Keyboard IME Insets in Compose"
slug: "android-keyboard-ime-insets-compose"
description: "WindowInsets.ime animation: bring fields into view, nested scroll, and edge-to-edge with keyboard open."
datePublished: "2026-07-23"
dateModified: "2026-07-23"
tags: ["Android", "Jetpack Compose", "IME"]
keywords: "IME insets Compose, keyboard insets animation, WindowInsets.ime Compose"
faq:
  - q: "When should Android teams adopt keyboard ime insets in compose?"
    a: "Adopt keyboard ime insets in compose when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with keyboard ime insets in compose?"
  ...
  - q: "How do I debug keyboard ime insets in compose issues in production?"
...
object AndroidKeyboardImeInsetsComposeFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-keyboard-ime-insets-compose_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for keyboard ime insets in compose
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-keyboard-ime-insets-compose` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get keyboard ime insets in compose wrong in predictable ways:
...
When keyboard ime insets in compose misbehaves in production:
...
Before enabling `android-keyboard-ime-insets-compose` for all users:
