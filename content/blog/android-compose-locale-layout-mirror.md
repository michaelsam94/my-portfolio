---
title: "Locale Layout Mirroring in Compose"
slug: "android-compose-locale-layout-mirror"
description: "RTL layouts: LayoutDirection, start/end modifiers, and testing Arabic/Hebrew mirroring without hard-coded left/right."
datePublished: "2026-07-17"
dateModified: "2026-07-17"
tags: ["Android", "Jetpack Compose", "RTL"]
keywords: "RTL Compose layout, LayoutDirection Rtl, locale mirroring Android"
faq:
  - q: "When should Android teams adopt locale layout mirroring in compose?"
    a: "Adopt locale layout mirroring in compose when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with locale layout mirroring in compose?"
  ...
  - q: "How do I debug locale layout mirroring in compose issues in production?"
...
object AndroidComposeLocaleLayoutMirrorFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-compose-locale-layout-mirror_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for locale layout mirroring in compose
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-compose-locale-layout-mirror` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get locale layout mirroring in compose wrong in predictable ways:
...
When locale layout mirroring in compose misbehaves in production:
...
Before enabling `android-compose-locale-layout-mirror` for all users:
