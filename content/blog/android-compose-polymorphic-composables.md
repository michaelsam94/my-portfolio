---
title: "Polymorphic Composables with Generics"
slug: "android-compose-polymorphic-composables"
description: "Generic list items, sealed UI models, and type-safe composable dispatch without runtime class checks everywhere."
datePublished: "2026-08-09"
dateModified: "2026-08-09"
tags: ["Android", "Jetpack Compose", "Kotlin"]
keywords: "polymorphic composables, generic Compose list, sealed class UI Compose"
faq:
  - q: "When should Android teams adopt polymorphic composables with generics?"
    a: "Adopt polymorphic composables with generics when you have production signals — Play Vitals regressions, ANR clusters, user-reported bugs, or security findings — and simpler fixes are exhausted. Pilot on one screen or user segment before platform-wide rollout, and measure cold start, jank, and crash rates before and after."
  - q: "What are the most common mistakes with polymorphic composables with generics?"
  ...
  - q: "How do I debug polymorphic composables with generics issues in production?"
...
object AndroidComposePolymorphicComposablesFeature {
    fun enabled(): Boolean =
        RemoteConfig.getBoolean("android-compose-polymorphic-composables_enabled", default = false)
}
...
class AndroidRepository @Inject constructor(
    ...
    suspend fun execute(): Result<Unit> = withContext(dispatcher) {
        runCatching {
            // Core logic for polymorphic composables with generics
        }
    }
}
```
...
Validate on API 26 and API 34+ hardware. Emulator-only testing misses `android-compose-polymorphic-composables` failures tied to exact alarm permission, photo picker backport behavior, and manufacturer-specific background limits.
...
Teams get polymorphic composables with generics wrong in predictable ways:
...
When polymorphic composables with generics misbehaves in production:
...
Before enabling `android-compose-polymorphic-composables` for all users:
