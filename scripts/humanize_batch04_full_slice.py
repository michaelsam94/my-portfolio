#!/usr/bin/env python3
"""Humanize batch-04 full slice indices 1000-1249 — >=1200 words, unique structures, topic FAQs."""
...
def slice_slugs() -> list[str]:
    files = sorted((ROOT / "content" / "blog").glob("*.md"))
    return [f.stem for f in files[1000:1250]]
...
def load_topics() -> dict[str, dict]:
    ...
    # Fallback: synthesize from existing frontmatter for non-devops / missing
    for slug in SLUGS:
        if slug in out:
            continue
        path = BLOG / f"{slug}.md"
        ...
        desc = grab("description", title)
        kw = grab("keywords", slug.replace("-", " "))
        tags = re.findall(r'-\s*"([^"]+)"', fm) or ["Engineering"]
        tech = title.split(":")[0].strip() if ":" in title else title
        out[slug] = {
            ...
            "tags": tags[:5],
            "keywords": kw,
            "hook": f"Teams treat {tech} as finished after the first green deploy — production disagrees.",
            "tech": tech,
            "when": f"When {tech} sits on a critical path for reliability, security, or cost.",
            "mistake": f"Copying tutorial defaults for {tech} without ownership, tests, or rollback.",
        }
    return out
...
def update_progress(slugs: list[str]):
    ...
    data["notes"] = f"Full slice 1000-1249 rewrite ({len(slugs)} posts)"
    data["status"] = "complete"
    data["stats"] = {"complete": len(slugs), "remaining": 0, "total": 250}
